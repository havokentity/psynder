...
- **ADR-016:** ✅ **Decided (2026-05-30).** **Capsule resting contacts emit a two-point MANIFOLD (segment/face clip), not a single re-collided point.** *Context.* The capsule narrowphase (`kernel_capsule_capsule`, the GJK capsule-box path, `kernel_plane_capsule`) emitted ONE deepest contact per step and re-collided every tick. One point is correct for a point-like / end-on contact, but a capsule lying on its SIDE (or two parallel capsules stacked, or a capsule resting flat on a box face) has a *line-like* contact region. Resolving an extended line with a single point gives the PGS solver no second point to supply a torque that resists rotation or lateral creep about the contact line, so the body slowly ROCKS / jitters / drifts as the single closest-point migrates from tick to tick (measured: a side-lying capsule and a stacked-capsule pair both slid >0.2 m laterally over 5 s; with the manifold the same cases drift <0.01 m). The kernel comments long flagged "a true two-point manifold is a Wave-B refinement". *Decision.* For the NEAR-PARALLEL configuration we clip the capsule's core segment against the other feature and emit a contact at BOTH ends of the overlap, each with its own per-point depth + the shared contact normal. (a) *capsule-capsule:* clip B's core segment onto A's core axis, emit at the two endpoints of the projected overlap interval. (b) *plane-capsule:* when the capsule axis lies in the surface plane, emit at both core endpoints, each with its own signed-distance depth. (c) *capsule-box:* when the recovered contact normal is cleanly aligned with a box FACE and the capsule axis is parallel to that face, clip the core segment to the face rectangle (parametric slab clip) and emit at both clipped ends. Parallelism is gated by `|sin(angle)| <= kManifoldParallelSin (0.04 rad)`; ANY non-parallel / point-like / end-on / cap-to-cap / non-face-aligned case returns exactly the LEGACY single point, so spheres, boxes, GJK pairs, and end-on capsule hits are byte-for-byte unchanged. This is the standard "reference-face clip → reduced manifold" technique (Catto, *Box2D* `b2CollidePolygons`; Coumans, Bullet persistent manifold), specialised to capsule cores. *Determinism + alloc-free (hard requirement, §10.1).* The manifold is a fixed-size `CapsuleManifold` (≤2 `ManifoldPoint`, no heap), built by pure arithmetic (no RNG/clock/atomics, `-fno-fast-math` friendly), so it reproduces bit-for-bit run-to-run (guarded by a 3-run bit-equality test). The island solver already consumes many contacts per body pair (a box stack does so), so the extra point flows through `detect_islands` / the colored solve untouched — no solver change. *Mechanics.* The clip algorithms live in a new `engine/physics/internal/CapsuleManifold.h`; the multi-contact dispatch `kernel_collide_pair_manifold` (fills a fixed `Contact[2]`, returns the count, delegating to the unchanged single-contact `kernel_collide_pair_spec` first so every speculative/separated/non-capsule pair is identical) lives in `engine/physics/internal/Kernels.h`. The ONLY other change is `World.cpp::run_narrowphase`, generalised from "push one contact" to "push the manifold's N contacts" — the documented narrowphase emission seam, mechanically the same loop. *Rejected alternatives.* (a) Keep one point but place it at the overlap centroid with averaged depth — stops the point *migrating* but still supplies no second torque, so the rocking/drift persists (measured: insufficient for the stacked-capsule case). (b) Persistent multi-frame manifold with contact-ID matching + warm-start carry-over — more state + a per-pair cache; unnecessary here because we re-collide every tick and the clip is deterministic, so the same two points reappear each step. (c) Full GJK/EPA contact manifold for all convex pairs — large and risky; this lane only needs the capsule resting cases the single-point kernels demonstrably mishandle. Guarded by `tests/unit/physics_capsule_manifold.cpp` (kernel-level point count/position/depth/normal for all three pairs; no-regression on end-on / perpendicular / cap-to-cap / standing cases; world-level flat-settle stability, stacked-capsule no-roll-off, end-on single-contact, and bit-for-bit determinism). See §10.1.

- **ADR-019:** ✅ **Decided (2026-05-31).** **The ragdoll primitive is a pure COMPOSITION of existing World bodies + the stock joint solver -- a skeleton of capsule/box segments connected by BALL-SOCKET pins, with limb inversion stopped by a ROPE soft cone limit and adjacent segments placed NON-OVERLAPPING; no new solver/constraint math.** *Context.* A death ragdoll needs an articulated multi-body figure (pelvis/torso/head + 2 arms + 2 legs, ~7 bodies) that collapses physically. The engine already has rigid bodies (`World::create_body` capsule/box) and a six-kind joint solver (`engine/physics/Joints.h`, run as a post-step pass via `solve_joints`). The open question was how to assemble them into a rag that drops, settles and stays connected without a bespoke articulated-body solver and without touching the contact solver / integrator / narrowphase. *Decision.* A ragdoll is just a `std::vector<BodyId>` + a `JointSet`, built by `physics::build_ragdoll` and driven by the documented loop `world.step(dt); solve_ragdoll(world, rag, dt);` (a thin wrapper over `solve_joints`). Each parent->child link is a `BallSocket` joint carrying full BodyIds (`joint_with_handles`, so a destroyed-and-recycled body can never be pulled by a dangling joint), with `rest_length` baked to the spawn anchor separation so the split-impulse position projection pins the two anchors coincident. The ball-socket alone leaves all three rotational DoF free (it inverts), so each link ALSO carries an additive `Rope` joint between OFF-AXIS anchors as a soft CONE LIMIT: the rope is slack inside the allowed cone half-angle and only pulls once the limb splays past it, catching the limb before it folds back through its parent -- a frictionless, energy-stable, one-sided limit reusing two stock joint kinds. *Why non-overlapping geometry (the load-bearing finding).* The engine has NO per-pair collision filtering, so two interpenetrating limb capsules generate contacts that the contact solver shoves apart EVERY tick while the joint pass pulls them back -- a permanent contact-vs-joint war that never settles (measured: ~7 cm chronic joint stretch + persistent jitter). The fix is to author every segment with a small POSITIVE surface gap at its joint (the joint anchors sit inside that gap, coincident), so the only contacts the rag ever generates are vs the ground. With that, the default humanoid drops, settles on a static plane with all joints within ~3 cm, and bleeds energy to rest (peak speed^2 a few m^2/s^2, decaying). *Determinism + alloc-free (hard requirement, §10.1).* `build_ragdoll` pre-sizes its id/joint arrays once; `solve_ragdoll` adds nothing beyond `solve_joints`' own pooled resolve scratch; teardown clears (keeps capacity). No RNG, no clock; inherits the lane `-fno-fast-math` gate, so a scripted spawn + sim reproduces bit-for-bit. Teardown is generation-safe (a stale id / already-freed body is a no-op) and idempotent. *Mechanics.* All in the new `engine/physics/Ragdoll.{h,cpp}`, using only the public World + joint surface -- `Physics.h`, `Joints.h`, the solver, the integrator and the narrowphase are UNCHANGED (purely additive lane). *Rejected alternatives.* (a) A dedicated reduced-coordinate / Featherstone articulated-body solver -- large, risky, and unnecessary when the existing maximal-coordinate joint pass already holds a 7-body tree. (b) Adding a true angular cone-twist constraint to the joint solver -- would change `Joints.h`'s solve math (out of the additive scope); the rope soft cone gives the needed anti-inversion behaviour with zero solver change. (c) Per-pair collision masks so overlapping limbs can co-exist -- a narrowphase change; non-overlapping authoring sidesteps it entirely and is also more physically honest. Guarded by `tests/unit/physics_ragdoll.cpp` (drops + settles on a plane staying connected; KE bounded + decreasing at rest, no energy gain; bit-for-bit determinism run-to-run; generation-safe teardown returns to baseline). See §10.8. *(Superseded in part by ADR-020: per-pair collision filtering now lets the ragdoll be authored anatomically tight; rejected-alternative (c) above is the path ADR-020 took.)*

- **ADR-020:** ✅ **Decided (2026-05-31).** **Per-pair collision filtering is a two-channel `(collision_group, collision_mask)` pair on the body, applied as a couple of integer ANDs in the broadphase pair-acceptance; the DEFAULT (all-ones / all-ones) means "collide with everything", so it is byte-for-byte additive.** *Context.* The engine had no way to say "these two bodies must not collide", which forced the Wave-11 ragdoll (ADR-019) to author artificial surface GAPS between linked limb segments: any overlap produced contacts the contact solver shoved apart every tick while the joint pass pulled them back -- a permanent contact-vs-joint war. A vehicle + its wheels, a character + its held prop, and many other "linked rigid set" cases hit the same wall. *Decision.* Each `Body` (and its `BodyDesc`) carries `u32 collision_group` + `u32 collision_mask`. Two bodies A,B are allowed to collide iff **each one's mask accepts the other's group**: `(A.mask & B.group) && (B.mask & A.group)` -- symmetric (the `&&` commutes), branch-predictable, reads only the two filter words already on the Body. The predicate lives header-only (`collision_filter_allows`, `engine/physics/Body.h`) so the tick and the unit tests share one definition. It is checked in the broadphase **pair-acceptance** (the `run_narrowphase` pair loop in `World.cpp`), BEFORE the narrowphase kernels, so a filtered-out pair never becomes a contact and costs nothing in island detect/solve downstream -- the narrowphase kernels, the colored solver, the integrator and the anti-tunnelling sweep are all UNCHANGED. A public `World::set_collision_filter(id, group, mask)` mutates it post-create (no static-body early-out: a static collider may legitimately filter which dynamics it accepts). *Why this scheme + this default (the load-bearing choices).* The group+mask pair is strictly more expressive than a single "same-group-id => skip" tag (it can model layers, one-way accept, and self-skip), yet costs the same two ANDs. The default is **all-ones for both** fields, which makes the predicate *unconditionally true*, so EVERY existing body / test / demo / golden pairs exactly as before -- the candidate-pair set is bit-identical and no resting stack / vehicle / capsule manifold / golden moves. Static-vs-static pairs were already inert (neither body moves; the solver no-ops on them) and the filter leaves that untouched. *Ragdoll tightening (the consumer).* `build_ragdoll` now stamps every segment with a shared self-collide group bit (`kRagdollSelfCollideGroup = 1<<30`, mask = `~that bit`), so the broadphase rejects all limb-vs-limb pairs within the rag while limb-vs-world pairs still collide (the world's default all-ones group contains the bit and its default mask accepts it). The default humanoid is therefore authored anatomically **tight** -- segments TOUCH at every joint, the ADR-019 inter-segment gaps are removed -- and still drops, settles on a static plane with all joints within ~3.5 cm, and bleeds energy to rest. (Tightening the rest pose bends limbs further into their `Rope` cone limits, so the default cone half-angles were widened to keep the soft-limit rope slack at rest -- otherwise the engaged rope slowly winches against the ball-socket pin.) *Determinism + cost (hard requirement, §10.1).* Integer-only, no RNG, no clock, zero per-frame heap; the check is a couple of ANDs in the existing pair loop. Inherits the lane `-fno-fast-math` gate -- a scripted sim reproduces bit-for-bit. *Rejected alternatives.* (a) A single "no-self-collision group id" tag -- simpler but can only express self-skip, not layers / one-way; the group+mask costs the same. (b) Filtering inside the narrowphase kernels -- would still pay the kernel call per filtered pair and touches kernel code; the pair-acceptance gate is cheaper and keeps the kernels frozen. (c) A separate per-pair exclusion set / hash -- per-frame allocation + a hash probe in the hot loop, vs two register ANDs. Guarded by `tests/unit/physics_collision_filter.cpp` (predicate correctness + symmetry; two overlapping boxes in the same no-collide group pass through while the default pair resolves; a default body still collides with everything; the public setter matches the desc; filtered sim is deterministic run-to-run; a tight self-filtered ragdoll settles stable) plus the unchanged `physics_ragdoll.cpp`. See §10.1, §10.8.

- **ADR-022:** ✅ **Decided (2026-05-31).** **The PsyGraph richer palette (Wave 13) is ADDED entirely within the frozen single-param node model — new ops keep `Instr` a flat 3-operand triple (3-input nodes lower to multiple 2-input ops), the seeded random is a STATELESS pure hash (not a stateful stream), and the graph blob format is UNCHANGED (no version bump); SUBGRAPHS are explicitly DEFERRED and live VM-debug hooks are shipped instead.** *Context.* Wave 13 grows the visual-scripting palette (more math/compare/vector/random/flow + a couple more ECS bindings) and asked for reusable named SUBGRAPHS. Two hard invariants constrain the design: (1) the browser serializer (`engine/editor/web/src/panels/psyGraphModel.ts`) and the C++ serializer (`Serialize.cpp`) must stay BYTE-IDENTICAL, and the browser writer emits exactly ZERO-OR-ONE inline `u64` param per node (`param_count = info.param === 'none' ? 0 : 1`); (2) the VM must stay deterministic and alloc-free per run (§10.1). *Decision — palette.* Every new node carries 0 or 1 inline params so both serializers agree node-for-node (verified byte-identical for a graph using `Once`+`RandomRange`+`Clamp`+`SetVelocity`). The new pure scalar ops (`Min`/`Max`/`Abs`/`Sign`/`Floor`/`Ceil`/`Sqrt`/`Mod`, `NotEqual`/`LessEqual`/`GreaterEqual`/`Xor`) map 1:1 to bytecode ops; the 3-input nodes do NOT add a 4-operand instruction — `Clamp(A,lo,hi)` lowers to `Min(Max(A,lo),hi)` and `Lerp(A,B,T)` to `A+(B-A)*T`, reusing the flat `Instr{op,a,b,c}` triple. `SetVelocity` needs 4 register args (entity + x,y,z); rather than widen `Instr`, the compiler `Move`s x,y,z into three freshly + CONSECUTIVELY allocated registers (`alloc_reg` is monotonic) and the VM reads `r[b], r[b+1], r[b+2]` from the single op. *Decision — seeded random.* `RandomRange(seed; Min, Max)` stays a PURE node: `seed` is a per-node inline param fed to a stateless SplitMix64 finalizer (`hash01`, fixed public-domain constants) producing a uniform double in `[0,1)`, then range-scaled with stock `Sub`/`Mul`/`Add`. Because it is a hash (not a stream), the same `(seed, Min, Max)` reproduces bit-for-bit across fresh programs/states — determinism is preserved with no RNG object and no clock. *Decision — stateful flow.* `Once` is a one-shot exec gate that reuses the existing per-instance variable bank: `param0` names a var slot, the lowering is `if (var[slot]) skip; else { var[slot]=true; <Then> }`, so it needs no new VM state — just `LoadVar`/`JumpIfFalse`/`StoreVar`. *Decision — ECS reads/writes.* `GetHealth` reads a component field back into the data graph via a new host getter `HostContext::get_health` (materialized like the event-data pins through a `LoadHealth` op, NOT constant-folded since it queries host state); `SetVelocity` writes through a new `HostContext::set_velocity`. An unset hook reads 0.0 / is a no-op, exactly like the existing action hooks. *Subgraphs DEFERRED (the load-bearing scoping call).* A node that invokes another named graph needs (a) a stable cross-graph asset reference in the blob, (b) input/output marshalling, and (c) a bounded/forbidden-recursion calling convention — none of which fit the current single-blob, single-param, no-asset-table format without a format version bump AND a matching browser-side asset registry, which is a materially larger change than the palette and would put the byte-identical-serializer invariant at risk for marginal Wave-13 value. Per the lane brief's prescribed fallback, subgraphs are DEFERRED; the shipped value for this commit is the full richer palette (math/compare/random/flow) plus the two new ECS bindings, which exhaust the high-value, byte-format-safe surface. Live VM-debug-overlay hooks (per-node last-values) are noted as the natural next step but are likewise out of this commit's scope. *Mechanics.* All additive in `engine/script/psygraph/*` (NodeTypes/Bytecode/Compiler/Vm/Host) + the browser mirror in `psyGraphModel.ts` + `PsyGraph.tsx` (palette auto-enumerates the catalog, so the new nodes appear without UI changes). No existing node id / op / blob byte moved; every pre-Wave-13 authored graph still compiles, runs, and round-trips identically. *Rejected alternatives.* (a) Widening `Value`/`Instr` to carry a Vec3 so vector nodes (make/break/length/normalize/dot) ship — breaks the 16-byte `Value` POD invariant and the byte layout both serializers depend on; deferred until a Value-layout ADR. (b) Adding a 4-operand instruction for `SetVelocity`/`Clamp`/`Lerp` — bloats every `Instr` and the serialized program; the Move-to-contiguous + lower-to-2-input approach keeps the triple. (c) A stateful RNG stream node (per-instance cursor) — would need new per-instance VM state and makes determinism order-dependent; the stateless seeded hash is reproducible by construction. Guarded by `tests/unit/psygraph_nodes.cpp` (each new node computes the right value in a compiled+run graph; `RandomRange` reproduces across fresh runs and stays in range; `Once` fires exactly once per instance; `GetHealth`/`SetVelocity` round-trip through host hooks; a graph using the new nodes round-trips byte-stable incl. the `Once` slot + `RandomRange` seed params; the pooled VM banks stay capacity-stable across 1000 runs) plus the unchanged `script_psygraph.cpp` + `scene_script_graph.cpp`. See §10.1.

- **ADR-023:** ✅ **Decided (2026-05-31).** **Squad flanking is an OPT-IN goal-OFFSET layer on top of the existing per-agent path: agents engaging a COMMON target are ranked by their stable entity id and each ranked agent's Chase goal is displaced to a DISTINCT angular slot on a ring around the target, so a squad surrounds/flanks instead of single-filing; the assignment is a pure deterministic function of the engaging id set (no RNG / clock), and with the flag off the goal is the raw target so behaviour is byte-for-byte the old single-agent path.** *Context.* Each AI agent perceived -> decided -> navigated to its target INDEPENDENTLY, so a group chasing the same entity all routed to the same cell and stacked up single-file down one corridor (the W12-3 separation nudge only un-stacks them locally, it does not spread the APPROACH). A tactical squad should pin from the front while the rest flank from the sides. The open question was how to add that coordination without a stateful "squad manager" object, without per-frame heap, and without changing perception / the FSM / A* / the host hooks — and keeping every existing single-agent host + test bit-for-bit unchanged. *Decision.* A header-only-config + free-function layer (`engine/ai/SquadCoord.{h,cpp}`) consulted ONLY from the goal-resolution step. Two agents are in the same SQUAD iff both are Chase/Attack and hold the same `target_entity`; the engaging set is discovered per-tick by a NESTED READ-ONLY query (the same re-entrant, alloc-free pattern `perceive`'s nearest-hostile and `act`'s `separation_nudge` already use), so it recomputes automatically as agents acquire / lose / switch targets. Within a squad each agent's RANK = the count of engaging members whose stable id (`Entity::index()`, the generation-free low bits) is strictly smaller — a total order on a unique-id set, so the rank, and hence the slot, is a pure function of the id set (same ids in => same slots out, byte-for-byte, independent of thread scheduling). Rank maps to a slot bearing that fans out symmetrically off an APPROACH AXIS (0, +a, -a, +2a, -2a, …); the axis is anchored to the direction from the target to the id-LOWEST member (a deterministic anchor, NOT a live centroid another worker is mutating), and the goal is that bearing rotated about world +Y and pushed out by `flank_radius`. A `suppressors` role split holds the N id-lowest members head-on (raw target goal) while the rest ring out — the classic "one pins, the rest flank". The offset is applied in BOTH the `navigate` pass (so A* routes to the flank slot) and the no-grid straight-line `act` fallback (so the grid-less df_demo soldiers also flank); a `count`=true flag on the `act` call makes it the single owner of the `ctx.flankers` telemetry so an agent offset in both passes is tallied once. *Why a goal-offset layer + this default (the load-bearing choices).* Offsetting only the GOAL means everything downstream — A* routing, string-pull smoothing, path following, the separation nudge, the host LOS/fire/move hooks — is reused verbatim; the squad layer adds no new system to the tick and no owned container. The master switch (`AiContext::squad.enabled`) DEFAULTS OFF, in which case `squad_flank_goal` returns the raw target immediately, so every existing host / unit test / golden takes the identical single-agent path (the no-regression test asserts the off-path goal equals the target exactly). *Determinism + alloc-free (hard requirement, §10.1).* Slot assignment is integer-rank + trig on fixed config; no RNG, no clock, no atomics in the assignment; the engaging-set scan reduces onto the caller's stack (zero per-frame heap), and each agent computes only its own goal (per-row, no cross-row mutation). The `df_demo` showcase enables it on the 6 terrain soldiers (close-in `attack_range` so they CHASE to flank before firing) and logs `squad: N agents assigned distinct flank slots, mean pairwise separation X m` (measured: 3 agents, ~18.7 m apart). *Rejected alternatives.* (a) A stateful SquadManager that owns squad membership + persistent slot leases — needs a registry/object lifetime + per-frame mutation the DOTS query model deliberately avoids; the per-tick id-ranked scan is stateless and recomputes for free. (b) Assigning slots by live position (nearest-angle-wins / centroid axis) — non-deterministic under parallel scheduling (positions are mid-write on other workers) and order-dependent; anchoring to a stable id removes both. (c) A new "flank" FSM state — would fork the perceive/think contract and the host hooks; offsetting the existing Chase goal needs neither. Guarded by `tests/unit/ai_squad.cpp` (3+ agents on one target get distinct id-sorted ranks/slots; their flank goals are spread around the target with mean pairwise separation well above the bunched baseline and each lands on a free nav cell; bit-for-bit determinism across repeated runs; the suppress-vs-flank split holds one head-on while the rest flank; layer-off goals equal the raw target — no regression; end-to-end `act()` fans a squad out more than the single-target baseline) plus the unchanged `ai_systems.cpp` (all 34 ai/nav cases pass with the layer off). See §10.7, §10.8.

- **ADR-024:** ✅ **Decided (2026-05-31).** **Combat audio is a one-way SIDE-CHANNEL: combat systems emit POD `AudioEvent {sound, position, volume}` into a pre-reserved queue a host drains into `audio::Engine::play`; the cues themselves are PROCEDURALLY synthesized (no asset files) by deterministic generators in the audio lane; gameplay never links the audio mixer.** *Context.* The engine has a spatial mixer (`audio/Audio.h` -- `play(ClipId, Vec3, vol)`) and a chip-synth, but combat (`fire_weapon` / hitscan hits / `ctx.deaths`) was silent. The combat module is deliberately "scene/physics/math only" (no audio link), and combat MUST stay bit-for-bit deterministic (goldens + the unit suite re-run identically), so audio could not be allowed to perturb any damage/death numeric, allocate in the hot path, or pull a clock/RNG. *Decision.* Two additive layers. (1) `engine/audio/CombatClips.{h,cpp}` synthesizes three mono cues -- a noise-dominated **gunshot**, a dull **impact/flesh-hit**, and a descending tonal **death groan** -- from a single stateful `ClipVoice` (LCG white noise + a sine body + a linear-attack / exp-decay envelope; the Death cue sweeps its tone down). The ONLY entropy is a per-sound fixed `seed`, so `synthesize_combat_clip` is a pure function: same descriptor -> byte-identical PCM. `register_combat_clips()` mints three stable, distinct `ClipId` identity tokens (ordinal+1) a host passes to `Engine::play` -- the mixer ignores clip sample data today (PCM streaming is a later wave), so the verifiable artifact is the synthesized PCM, which the tests exercise directly; when streaming lands the same generators feed the mixer with no call-site change. (2) `engine/gameplay/CombatAudio.{h,cpp}` owns a `CombatAudioQueue` (pre-reserved `std::vector<AudioEvent>`, `begin_tick()` clears + retains capacity) and emit helpers: `emit_fire` (gunshot at the muzzle, gated on the shot actually going out so a refused cooldown/no-ammo shot makes no sound and there is no double-emit), `emit_hit` (impact at `origin + dir*HitResult.distance`), `emit_shot` (both), and `emit_death` / `emit_deaths` (death cue at each `ctx.deaths` corpse, read from its `TransformComponent` BEFORE `resolve_deaths` despawns it). Helpers only READ the registry for positions; they never touch a `HealthComponent`, the damage/death lists, ammo, or cooldowns. *Why a side-channel + a gameplay-local sound enum (the load-bearing choices).* Keeping the queue + a gameplay-owned `CombatSoundId` (mirroring `audio::CombatSound` by ordinal) means `psynder_gameplay` does NOT gain an `audio` link -- the layering stays clean and the combat module remains testable without the mixer. The host is the single bridge: drain the queue, map each `sound` to a `ClipId` via the registered table, call `play(clip, ev.position, ev.volume)`. Because emission is downstream of (and disjoint from) every combat write, the combat result is provably independent of whether audio is enabled. *Determinism + alloc-free (hard requirement, §10.1).* Clip synthesis is integer-LCG + analytic envelope math, no clock/RNG-by-time, no heap (the caller owns the PCM buffer); the queue reserves once and only `clear()`s. *Rejected alternatives.* (a) Calling `Engine::play` directly from `fire_weapon` -- would force a gameplay->audio link, start a real audio device inside headless combat ticks, and entangle a deterministic system with a stateful global mixer. (b) Shipping WAV/OGG combat samples -- needs asset files + a loader + a clip table the mixer doesn't have yet; procedural cues are zero-dependency and deterministic. (c) An `audio::CombatSound` referenced from gameplay -- would link audio into the combat module for nothing more than an enum. Guarded by `tests/unit/combat_audio.cpp` (clips non-empty + in-range + byte-identical run-to-run + distinct per cue; fire -> gunshot at the shooter; hit -> impact at the reconstructed hit point; a refused shot + a miss emit no impact / no double-emit; death -> death cue at the corpse; and the keystone: an identical 4-shot combat sequence run with the full audio side-channel vs. with it untouched yields a BYTE-IDENTICAL health/death log). The demo is intentionally NOT wired (no demo starts an audio device; doing so would risk the headless smoke runner) -- the headless verification is the unit test, not literal sound. See §10.2.
