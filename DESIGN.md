...
- **Solver:** sequential-impulse / projected Gauss-Seidel **per island, in parallel across islands**. 8 velocity iterations + 3 position iterations by default. Warm-started across frames.
...
phys_tick():
  ...
  parallel: per-island solver                 [1 island / job]
...
3. **§16 ADR log** — the twelve ADR slots are the load-bearing answers. Each one closes off a question a contributor would otherwise relitigate. Ten decided (1, 2, 3, 4, 7, 8, 9, 10, 11, 12), two not-applicable (5, 6).
