...
## Wave 6 (in flight)
- Editor no-code gameplay authoring (editor/web/scene/PlayRuntime) — Health/Weapon/Faction/Hitbox/AI components in the inspector + SceneFile serialization + ColliderShape::Plane + authored-into-Play. [IN FLIGHT — lane a9830f7955caf0ea7]
...
- Render perf kernelization, bench-gated (engine/render). [LANDED 199f0b4 — raster fragment hoist + RT closest-hit normal-deferral; raster_unlit -17.8%/lit -5.0%/rt_bvh -3.4%; goldens BIT-IDENTICAL (FNV oracle), debug 892 x3, zero new alloc, new bench target psynder_bench_render_hot.]
- RT telemetry state-leak fix (engine/render/rt) — fixing a PRE-EXISTING release-only order-dependent unit failure (see below). [IN FLIGHT — lane a5238ec886228e79e]
...
- Found prior-session editor-authoring WIP uncommitted in the MAIN worktree (GameplayComponents.h untracked + dirty WebPanels/PlayRuntime/SceneFile/player main + scene_authoring_roundtrip test). It was NEVER committed (reflog clean) and did NOT contaminate any pushed commit (cherry-picks apply specific diffs). Stashed safely: `stash@{0}` (this session's set-aside) + `stash@{1}` (older "real-channel integration wip" backup). The Wave-6 editor lane a9830f is building this feature cleanly from bf35478; at integration, compare its diff vs the stashes and keep the cleaner one, then drop the stashes.
