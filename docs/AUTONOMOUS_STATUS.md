...
HEAD = `725f75c` on `integration/wave-hybrid-material-shadows`, pushed + in sync, working tree CLEAN (only untracked `.claude/`). Gated checks GREEN at HEAD: release `psynder_unit` 0-failed (914 cases) x3, debug 0-failed (913) x3, goldens 4/4, debug+release builds 0 error, arcade relinks, all 3 demos exit 0 in debug AND release, release arcade smoke exits 0. Waves 5 + 6 COMPLETE. Wave 7: AOI netcode (f63d848) + PsyGraph no-code scripting (5b262e3) + **demo teardown SIGABRT fix (725f75c)** LANDED.

1. **[FIXED 725f75c] Demo teardown SIGABRT (exit 134).** Root cause confirmed exactly as predicted: the `this`-keyed `StateRegistry` (engine/render/rt/Bvh.cpp) was a function-local static holding a `std::mutex`; a `Bvh8`/`Tlas` destroyed during teardown AFTER the registry static -> erase locks a destroyed mutex -> libc++ throws `std::system_error` ("mutex lock failed: Invalid argument") from a noexcept dtor -> terminate -> SIGABRT. FIX = leak-on-exit heap singletons (registry never destroyed; only static is a raw ptr -> no atexit hook; map reclaimed by OS). REGRESSION = namespace-scope guard in tests/unit/render_rt_frame_helpers.cpp builds+tears down a BLAS+TLAS from its dtor (runs at exit after the registry); old code -> unit binary passes all cases but exits 134, fixed -> exits 0. PROVEN both ways (stashed the fix -> saw 134; restored -> saw 0). All demo smokes (df/racer/shooter) + arcade now exit 0 in debug+release.

2. **[NEXT — now UNBLOCKED] Wave 7 vehicle-on-terrain demo wiring (#80).** WIP preserved on branch `wip/vehicle-on-terrain` (@ `7e4a0e5`, games/ only). Status: df_demo drivable jeep terrain-follow LOGIC works ("TRACKS-TERRAIN GROUNDED ... PASS", 2.29m chassis-Y span over slope); the exit-134 abort that blocked it is now FIXED on integration. Remaining: rebase/cherry-pick the WIP onto integration (HEAD 725f75c), retune the racer governor (it swung from runaway to UNDER-CAP STALLED: peak 10.4 m/s vs 12 cap, 32.7m, 0 laps), re-verify both df_demo + racer_demo smokes exit 0 with intended motion. Wave-6 ENGINE physics (set_ground_heightfield + governor, #58) is DONE + shipped (452bb3a); only the games/ demo wiring is pending.
