...
HEAD = `064f8f4` on `integration/wave-hybrid-material-shadows`, pushed + in sync, working tree CLEAN (only untracked `.claude/`). Gated checks GREEN at HEAD: debug `psynder_unit` 0-failed (914 cases), release 0-failed (915), goldens 4/4, debug+release builds 0 error, arcade relinks, all 3 demos exit 0 in debug AND release, release arcade smoke exits 0. Waves 5 + 6 COMPLETE. **Wave 7 COMPLETE**: AOI netcode (f63d848) + PsyGraph (5b262e3) + demo teardown SIGABRT fix (725f75c) + **Pacejka lateral tire-force sign fix (bda47cb)** + **vehicle-on-terrain demo wiring #80 (064f8f4)** LANDED.

Both prior-session open items are now DONE:
...
NEXT = Wave 8 (from the heartbeat plan), one file-disjoint lane at a time:
- ECS query kernelization (engine/scene, bench-gated, goldens bit-identical).
- Physics solver SIMD (engine/physics, bench-gated, goldens bit-identical).
- Editor vehicle-on-terrain authoring (expose set_ground_heightfield + VehicleDesc governor/steer fields in the Inspector so a designer can place a drivable vehicle on terrain no-code; PlayRuntime already synthesizes gameplay components).
Also opportunistic: GJK/box-box speculative-contact coverage; prune stale agent worktrees (~40 locked under .claude/worktrees + .worktrees, ~1GB each; `git worktree remove -f -f`); delete the superseded `wip/vehicle-on-terrain` branch.
