...
bool read_mesh(std::span<const u8> bytes, Mesh& mesh) {
    formats::LmmHeader hdr{};
    if (!read_pod(bytes, 0, hdr))
        return false;
    if (hdr.file.magic != formats::kLmmMagic)
        return false;
    if (hdr.file.version != formats::kLmmVersion)
        return false;
    if (hdr.vertex_stride == 0)
        return false;
...
bool write_texture(std::vector<u8>& out, const Texture& texture) {
    ...
    for (const auto& mip : texture.mips) {
        ...
        if (end > texture.pixel_data.size())
            return false;
    }
    if (texture.pixel_fmt == formats::LmtPixelFmt::P8) {
        if (texture.palette.size() != 256u * 4u)
            return false;
    }
    if (texture.mips.size() > 16)
        return false;  // matches LmtHeader::mip_count
    ...
    const usize payload_after_header = mip_table_bytes + palette_bytes + texture.pixel_data.size();
...
bool read_texture(std::span<const u8> bytes, Texture& texture) {
    ...
    if (!read_pod(bytes, 0, hdr))
        return false;
    if (hdr.file.magic != formats::kLmtMagic)
        return false;
    if (hdr.file.version != formats::kLmtVersion)
        return false;
    if (hdr.mip_count == 0 || hdr.mip_count > 16)
        return false;

    const usize mip_table_off = sizeof(hdr);
    const usize mip_table_bytes = static_cast<usize>(hdr.mip_count) * sizeof(formats::LmtMip);
    if (mip_table_off + mip_table_bytes > bytes.size())
        return false;
    ...
    if (pixels_off == 0 || pixels_off > bytes.size())
        return false;

    for (u8 i = 0; i < hdr.mip_count; ++i) {
        formats::LmtMip wire{};
        if (!read_pod(bytes, mip_table_off + i * sizeof(wire), wire))
            return false;
        if (wire.offset < pixels_off)
            return false;
    ...
    if (hdr.palette_offset != 0) {
        ...
        if (pal_off + pal_bytes > bytes.size())
            return false;
...
bool write_audio(std::vector<u8>& out, const Audio& audio) {
    ...
    const usize expect_bytes = static_cast<usize>(audio.frame_count) * audio.channels * bps;
    if (audio.pcm_data.size() != expect_bytes)
        return false;
    ...
    if ((audio.flags & formats::kLmaFlagLoop) != 0) {
        if (audio.loop_end > audio.frame_count)
            return false;
        if (audio.loop_start > audio.loop_end)
            return false;
    }
...
bool read_audio(std::span<const u8> bytes, Audio& audio) {
    ...
    if (!read_pod(bytes, 0, hdr))
        return false;
    if (hdr.file.magic != formats::kLmaMagic)
        return false;
    if (hdr.file.version != formats::kLmaVersion)
        return false;
    if (hdr.channels == 0)
        return false;

    const u32 bps = bytes_per_sample(hdr.sample_fmt);
    const usize expect_bytes = static_cast<usize>(hdr.frame_count) * hdr.channels * bps;
    if (sizeof(hdr) + expect_bytes > bytes.size())
        return false;
