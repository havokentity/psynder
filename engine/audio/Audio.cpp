...
void mixer_pull(f32* out_stereo, u32 frames, void* /*user*/) noexcept {
    ...
    std::fill(s.scratch_stereo.begin(), s.scratch_stereo.begin() + stereo_floats, 0.0f);
    std::memset(out_stereo, 0, sizeof(f32) * stereo_floats);
    ...
    if (snapshot_count > 0u) {
        ...
        js.parallel_for(0u, snapshot_count, 1u, [&](usize begin, usize end) {
            for (usize vi = begin; vi < end; ++vi) {
                ...
                const f32 doppler = detail::doppler_ratio(listener_eye,
                                                          listener_vel,
                                                          snapshot[vi].position,
                                                          snapshot[vi].velocity);
                ...
                detail::render_voice_into_stereo(snapshot[vi], 440.0f, s.desc.sample_rate, frames, buf, doppler);
            }
        });
        ...
        // bump cursors under lock so next pull continues smoothly.
        {
            std::lock_guard<std::mutex> lk(s.voices_mu);
            for (u32 i = 0; i < detail::kMaxVoices; ++i) {
                if (s.voices.at(i).active)
                    s.voices.at(i).cursor += frames;
            }
        }
    }
...
bool Engine::start(const DeviceDesc& desc) {
    ...
    s.desc = desc;
    if (s.desc.channels != 2u)
        s.desc.channels = 2u;  // mixer is stereo for Wave A
    if (s.desc.sample_rate == 0u)
        s.desc.sample_rate = 48000u;
    if (s.desc.buffer_frames == 0u)
        s.desc.buffer_frames = 512u;
    ...
    s.per_voice_buffers.assign(static_cast<usize>(detail::kMaxVoices) * 2u *
                                   static_cast<usize>(s.desc.buffer_frames),
                               0.0f);
    ...
    s.fdn_mod.reset(s.desc.sample_rate,
                    /*decay s*/ 2.5f,
    ...
    s.indoor_partitioned_l.reset(s.desc.sample_rate,
                                 /*ir s*/ 0.40f,
                                 /*decay s*/ 1.2f,
                                 conv_block,
                                 /*overlap*/ 4u,
                                 /*seed*/ 0xC0FFEEu);
    s.indoor_partitioned_r.reset(s.desc.sample_rate,
                                 /*ir s*/ 0.40f,
                                 /*decay s*/ 1.2f,
                                 conv_block,
                                 /*overlap*/ 4u,
    ...
    PSY_LOG_INFO("[audio] Engine started — backend={}, sr={}, frames={}",
                 static_cast<int>(s.chosen_backend),
                 s.desc.sample_rate,
                 s.desc.buffer_frames);
    return true;
}
