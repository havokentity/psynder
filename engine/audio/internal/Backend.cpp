...
bool backend_init(const DeviceDesc& desc, MixerCallback cb, void* user, Backend& chosen) noexcept {
    ...
    switch (b) {
        case Backend::WASAPI:
            return backend_init_wasapi(desc, cb, user);
        case Backend::CoreAudio:
            return backend_init_coreaudio(desc, cb, user);
        case Backend::PipeWire:
            return backend_init_pipewire(desc, cb, user);
        case Backend::ALSA:
            return backend_init_alsa(desc, cb, user);
        case Backend::Auto:
...
void backend_shutdown(Backend chosen) noexcept {
    switch (chosen) {
        case Backend::WASAPI:
            backend_shutdown_wasapi();
            break;
        case Backend::CoreAudio:
            backend_shutdown_coreaudio();
            break;
        case Backend::PipeWire:
            backend_shutdown_pipewire();
            break;
        case Backend::ALSA:
            backend_shutdown_alsa();
            break;
        case Backend::Auto:
            break;
    }
