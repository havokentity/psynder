...
inline void itd_to_delays(f32 azimuth_rad, u32 sample_rate, u32& left_delay, u32& right_delay) noexcept {
...
inline StereoHrir make_minimal_hrir(f32 azimuth_rad, u32 sample_rate) noexcept {
    ...
    for (u32 i = 0; i < kBaseTaps; ++i) {
        base[i] = 0.5f * (1.0f - std::cos(2.0f * math::kPi * (static_cast<f32>(i) + 0.5f) /
...
inline void fft(f32* data, u32 log2n) noexcept {
...
class FftConvReverb {
   public:
    ...
    u32 process_block(const f32* in, u32 n, f32* out) noexcept {
        ...
        u32 log2n = 1;
        while ((1u << log2n) < conv_len)
            ++log2n;
        const u32 fft_n = 1u << log2n;
        if (fft_n * 2u > kScratch)
            return 0u;
...
PSY_FORCEINLINE void render_voice_into_stereo(const Voice& v,
                                              f32 freq_hz,
                                              u32 sample_rate,
                                              u32 frames,
    ...
    lg *= v.volume;
    rg *= v.volume;
    const f32 omega = 2.0f * math::kPi * freq_hz * doppler_ratio / static_cast<f32>(sample_rate);
