...
class PartitionedConvolver {
   public:
    ...
    void reset(u32 sample_rate,
        ...
        if (block_size < kMinBlock)
            block_size = kMinBlock;
        if (block_size > kMaxBlock)
            block_size = kMaxBlock;
        ...
        ir_.assign(static_cast<usize>(num_partitions_) * static_cast<usize>(block_), 0.0f);
        ...
        partition_spec_.assign(static_cast<usize>(num_partitions_) * static_cast<usize>(2u * fft_len_),
                               0.0f);
        for (u32 p = 0; p < num_partitions_; ++p) {
            f32* spec =
                partition_spec_.data() + static_cast<usize>(p) * static_cast<usize>(2u * fft_len_);
        ...
        frame_ring_.assign(static_cast<usize>(num_partitions_) * static_cast<usize>(2u * fft_len_),
                           0.0f);
        ring_head_ = 0;
    ...
    void process_block_into(const f32* in, f32* out) noexcept {
        ...
        if (overlap_ == 1u) {
            ...
            f32* head =
                frame_ring_.data() + static_cast<usize>(ring_head_) * static_cast<usize>(2u * N);
            ...
            for (u32 p = 0; p < num_partitions_; ++p) {
                ...
                const f32* X =
                    frame_ring_.data() + static_cast<usize>(slot) * static_cast<usize>(2u * N);
                const f32* H =
                    partition_spec_.data() + static_cast<usize>(p) * static_cast<usize>(2u * N);
                for (u32 i = 0; i < N; ++i) {
        ...
        for (u32 h = 0; h < overlap_; ++h) {
            ...
            for (u32 i = 0; i < B; ++i) {
                ...
                fft_scratch_[2 * i + 0] = (sample_offset + i < B) ? prev_input_[sample_offset + i]
   ...
   public:
    void reset(u32 sample_rate,
        ...
        for (u32 i = 0; i < kFdnSize; ++i) {
            ...
            mod_step_[i] = math::kTwoPi * mod_rate_hz_base * mult[i] / static_cast<f32>(sample_rate);
        }
    ...
    PSY_FORCEINLINE f32 tick(f32 in) noexcept {
        ...
        for (u32 i = 0; i < kFdnSize; ++i) {
            ...
            const f32 read_f = static_cast<f32>(head_[i] + kMaxLine - line_len_[i]) + offs;
            ...
            if (mod_phase_[i] > math::kTwoPi)
                mod_phase_[i] -= math::kTwoPi;
        }
