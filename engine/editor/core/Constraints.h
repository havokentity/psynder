...
PSY_FORCEINLINE Constraint
make_axis(u32 a, u32 b, math::Vec3 anchor, math::Vec3 axis, f32 min_angle, f32 max_angle) noexcept {
    Constraint c;
    c.kind = Kind::Axis;
    c.body_a = a;
    c.body_b = b;
    ...
    c.min_limit = min_angle;
    c.max_limit = max_angle;
    return c;
}

PSY_FORCEINLINE Constraint
make_slider(u32 a, u32 b, math::Vec3 anchor, math::Vec3 axis, f32 min_t, f32 max_t) noexcept {
    Constraint c;
    c.kind = Kind::Slider;
    c.body_a = a;
    c.body_b = b;
...
PSY_FORCEINLINE Constraint make_elastic(
    u32 a, u32 b, math::Vec3 ax, math::Vec3 bx, f32 rest_length, f32 stiffness, f32 damping) noexcept {
    Constraint c;
    c.kind = Kind::Elastic;
    c.body_a = a;
    c.body_b = b;
