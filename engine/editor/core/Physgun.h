...
PSY_FORCEINLINE bool is_active(const State& s) noexcept {
    return s.active && s.body_id != 0;
}
...
PSY_FORCEINLINE bool ray_aabb_intersect(
    math::Vec3 origin, math::Vec3 dir, math::Vec3 centre, math::Vec3 he, f32& t_near) noexcept {
...
PSY_FORCEINLINE PickResult pick(std::span<const PickInput> bodies,
    ...
    for (const PickInput& b : bodies) {
        if (b.id == 0)
            continue;
        f32 t = 0.0f;
        if (!ray_aabb_intersect(origin, dir, b.position, b.half_extent, t))
            continue;
        if (t < best_t) {
            best_t = t;
            best.body_id = b.id;
