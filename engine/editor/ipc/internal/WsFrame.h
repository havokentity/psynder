...
inline std::vector<::psynder::u8> encode_server(Op op, const ::psynder::u8* data, ::psynder::usize n, bool fin = true) {
    ...
    if (n <= 125) {
        ...
        out.push_back(127);
        ::psynder::u64 v = n;
        for (int i = 7; i >= 0; --i) out.push_back(static_cast<::psynder::u8>(v >> (i*8)));
    }
...
inline std::vector<::psynder::u8> encode_close(::psynder::u16 code, std::string_view reason) {
    ...
    for (char c : reason) body.push_back(static_cast<::psynder::u8>(c));
...
inline ::psynder::usize try_parse_client(const ::psynder::u8* in, ::psynder::usize n, Frame& frame) {
    ...
    if (!masked) return SIZE_MAX;  // client MUST mask
    ...
    if (len == 126) {
        ...
        if (n < p + 8) return 0;
        len = 0;
        for (::psynder::usize i = 0; i < 8; ++i) len = (len << 8) | in[p+i];
        p += 8;
    }
    if (n < p + 4) return 0;
    ...
    if (len > (16ull * 1024 * 1024)) return SIZE_MAX;
