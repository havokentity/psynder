...
f32 determinant(const Mat3& m) noexcept {
    ...
    return m.m[0] * (m.m[4] * m.m[8] - m.m[5] * m.m[7]) -
           m.m[1] * (m.m[3] * m.m[8] - m.m[5] * m.m[6]) + m.m[2] * (m.m[3] * m.m[7] - m.m[4] * m.m[6]);
}

Mat3 inverse(const Mat3& m) noexcept {
    f32 det = determinant(m);
    if (det == 0.0f)
        return identity3();  // singular — fall back
...
Quat quat_nlerp(Quat a, Quat b, f32 t) noexcept {
    ...
    Quat bb = quat_dot(a, b) < 0.0f ? Quat{-b.x, -b.y, -b.z, -b.w} : b;
...
Vec3 quat_to_euler(Quat q) noexcept {
    ...
    f32 roll = std::atan2(2.0f * (q.w * q.x + q.y * q.z), 1.0f - 2.0f * (q.x * q.x + q.y * q.y));
    f32 pitch = std::asin(sinp);
    f32 yaw = std::atan2(2.0f * (q.w * q.z + q.x * q.y), 1.0f - 2.0f * (q.y * q.y + q.z * q.z));
