...
class HostImpl {
   public:
    ...
    void send(PeerId peer, std::span<const u8> bytes, bool reliable, u8 channel = kChannelDefault) noexcept;
   ...
   private:
    ...
    void send_raw_(u16 dst_port, const FrameHeader& h, std::span<const u8> payload) noexcept;

    void handle_reliable_in_(PeerState& ps, const FrameHeader& h, std::span<const u8> payload) noexcept;
    void handle_unreliable_in_(PeerState& ps, const FrameHeader& h, std::span<const u8> payload) noexcept;
