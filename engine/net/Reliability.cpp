...
PSY_FORCEINLINE u32 try_retire(std::array<SendEntry, kSendRingSlots>& ring,
                               u32 seq, u32& inflight) noexcept {
...
Reliability::Reliability(WindowSize sz) noexcept
...
u32 Reliability::apply_ack_wide(u32 ack_base,
                                u32 bits_lo, u32 bits_hi,
                                u32 bits_hi2, u32 bits_hi3) noexcept {
    ...
    for (u32 i = 0; i < max_i; ++i) {
        ...
        if ((words[word] & (1u << shift)) == 0) continue;
        if (i + 1 > ack_base) continue;   // 0 reserved
...
bool AckTracker::observe(u32 seq) noexcept {
    ...
    if (i < 64) {
        bit = u64(1) << i;
        if (bits_[0] & bit) return false;
        bits_[0] |= bit;
    } else {
        bit = u64(1) << (i - 64);
        if (bits_[1] & bit) return false;
        bits_[1] |= bit;
    }
    return true;
}
