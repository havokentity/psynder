...
bool decode_snapshot(std::span<const u8> in, SnapshotFrame& out) noexcept {
    if (in.size() < kSnapshotHeaderBytes)
        return false;
...
void interpolate(const SnapshotFrame& a, const SnapshotFrame& b, f32 t, SnapshotFrame& out) noexcept {
    ...
    for (const SnapshotEntity& ea : a.entities) {
        auto it = std::find_if(b.entities.begin(), b.entities.end(), [&](const SnapshotEntity& x) {
    ...
    for (const SnapshotEntity& eb : b.entities) {
        auto it = std::find_if(a.entities.begin(), a.entities.end(), [&](const SnapshotEntity& x) {
