...
public:
    ...
    void set_title(std::string_view t) override {
        title_ = std::string{t};
        if (xdg_toplevel_) xdg_toplevel_set_title(xdg_toplevel_, title_.c_str());
    }
...
bool WaylandWindow::init(const WindowDesc& desc) noexcept {
    ...
    if (!registry_) { teardown(); return false; }
    wl_registry_add_listener(registry_, &kRegistryListener, this);

    // Roundtrip 1: receive globals.
    if (wl_display_roundtrip(display_) < 0) { teardown(); return false; }
    ...
    if (!surface_) { teardown(); return false; }
    ...
    while (!configured_) {
        if (wl_display_dispatch(display_) < 0) { teardown(); return false; }
    }
    return true;
}
...
void WaylandWindow::teardown() noexcept {
    ...
    if (xdg_surf_)        { xdg_surface_destroy(xdg_surf_);        xdg_surf_     = nullptr; }
    ...
    if (globals_.shm)     { wl_shm_destroy(globals_.shm);          globals_.shm  = nullptr; }
    if (globals_.compositor) { wl_compositor_destroy(globals_.compositor); globals_.compositor = nullptr; }
...
bool WaylandWindow::ensure_shm_for(u32 w, u32 h) noexcept {
    ...
    if (needed <= shm_size_ && shm_data_ && shm_buffer_) return true;
...
bool WaylandWindow::allocate_shm_pool(u32 w, u32 h) noexcept {
    ...
    if (!pool) { ::munmap(map, size); ::close(fd); return false; }
    ...
    if (!buf) { wl_shm_pool_destroy(pool); ::munmap(map, size); ::close(fd); return false; }
...
void WaylandWindow::present(const render::Framebuffer& fb) {
    ...
    if (fb.width != render_w_ || fb.height != render_h_) {
        render_w_ = fb.width;
        render_h_ = fb.height;
        if (!allocate_shm_pool(render_w_, render_h_)) return;
    }
    ...
    if (viewport_) {
        ...
        const BlitRect r = compute_blit_rect(
            static_cast<int>(window_w_), static_cast<int>(window_h_),
            static_cast<int>(render_w_),  static_cast<int>(render_h_),
            aspect_, scale_mode_);
        const int dst_w = r.w > 0 ? r.w : static_cast<int>(window_w_);
        const int dst_h = r.h > 0 ? r.h : static_cast<int>(window_h_);
        wp_viewport_set_destination(viewport_, dst_w, dst_h);
...
// ─── Listener bodies ─────────────────────────────────────────────────────
void WaylandWindow::on_registry(void* data, wl_registry* reg, uint32_t name,
    ...
    if (std::strcmp(iface, wl_compositor_interface.name) == 0) {
        ...
        w->globals_.shm = static_cast<wl_shm*>(
        ...
        w->globals_.wm_base = static_cast<xdg_wm_base*>(
            wl_registry_bind(reg, name, &xdg_wm_base_interface,
                             std::min<uint32_t>(version, 5u)));
    } else if (std::strcmp(iface, wp_viewporter_interface.name) == 0) {
        w->globals_.viewporter = static_cast<wp_viewporter*>(
...
void WaylandWindow::on_xdg_surface_configure(void* data, xdg_surface* surf,
                                              uint32_t serial) {
    auto* w = static_cast<WaylandWindow*>(data);
...
void WaylandWindow::on_toplevel_configure(void* data, xdg_toplevel*,
                                           int32_t w_, int32_t h_, wl_array*) {
    auto* w = static_cast<WaylandWindow*>(data);
    if (w_ > 0) w->window_w_ = static_cast<u32>(w_);
    if (h_ > 0) w->window_h_ = static_cast<u32>(h_);
}
...
// ─── Keyboard ─────────────────────────────────────────────────────────────
void WaylandWindow::on_keyboard_keymap(void* data, wl_keyboard*, uint32_t format,
    ...
    ::close(fd);
    if (km_str == MAP_FAILED) return;
    xkb_keymap* km = xkb_keymap_new_from_string(
        w->xkb_ctx_, km_str, XKB_KEYMAP_FORMAT_TEXT_V1, XKB_KEYMAP_COMPILE_NO_FLAGS);
    ::munmap(km_str, size);
    if (!km) return;
    if (w->xkb_keymap_) xkb_keymap_unref(w->xkb_keymap_);
    if (w->xkb_state_)  xkb_state_unref(w->xkb_state_);
    w->xkb_keymap_ = km;
    w->xkb_state_  = xkb_state_new(km);
}
void WaylandWindow::on_keyboard_enter(void*, wl_keyboard*, uint32_t,
                                       wl_surface*, wl_array*) {}
void WaylandWindow::on_keyboard_leave(void*, wl_keyboard*, uint32_t, wl_surface*) {}

void WaylandWindow::on_keyboard_key(void* data, wl_keyboard*, uint32_t /*serial*/,
...
void WaylandWindow::on_keyboard_modifiers(void* data, wl_keyboard*, uint32_t,
    ...
    auto* w = static_cast<WaylandWindow*>(data);
    if (w->xkb_state_) {
        xkb_state_update_mask(w->xkb_state_, mods_depressed, mods_latched,
...
// ─── Pointer ──────────────────────────────────────────────────────────────
void WaylandWindow::on_pointer_enter(void*, wl_pointer*, uint32_t,
                                      wl_surface*, wl_fixed_t, wl_fixed_t) {}
void WaylandWindow::on_pointer_leave(void*, wl_pointer*, uint32_t, wl_surface*) {}

void WaylandWindow::on_pointer_motion(void* /*data*/, wl_pointer*, uint32_t,
...
void WaylandWindow::on_pointer_button(void*, wl_pointer*, uint32_t /*serial*/,
                                       uint32_t /*time*/, uint32_t button,
                                       uint32_t state) {
    int b = -1;
    switch (button) {
    case evdev::BTN_LEFT:   b = 0; break;
    case evdev::BTN_RIGHT:  b = 1; break;
    case evdev::BTN_MIDDLE: b = 2; break;
    default: return;
    }
    input_push_mouse_button(b, state == WL_POINTER_BUTTON_STATE_PRESSED);
}

void WaylandWindow::on_pointer_axis(void*, wl_pointer*, uint32_t,
