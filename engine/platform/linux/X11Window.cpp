...
bool X11Window::init(const WindowDesc& desc) noexcept {
    ...
    Bool xshm_ok = XQueryExtension(dpy_, "MIT-SHM",
                                   &xshm_major_opcode,
                                   &xshm_evt_base_,
                                   &xshm_err_base_);
    if (!xshm_ok) {
    ...
    win_ = XCreateWindow(
        dpy_, RootWindow(dpy_, screen_),
        0, 0, window_w_, window_h_,
        0, CopyFromParent, InputOutput, visual,
        CWEventMask | CWBackPixel | CWBorderPixel, &attrs);
    if (!win_) { teardown(); return false; }
...
bool X11Window::allocate_xshm(u32 w, u32 h) noexcept {
    ...
    ximage_ = XShmCreateImage(dpy_, visual, depth, ZPixmap,
                               nullptr, &shm_info_,
    ...
    if (shm_info_.shmid < 0) { free_xshm(); return false; }

    shm_info_.shmaddr = static_cast<char*>(shmat(shm_info_.shmid, nullptr, 0));
    if (shm_info_.shmaddr == reinterpret_cast<char*>(-1)) { free_xshm(); return false; }
    ximage_->data       = shm_info_.shmaddr;
    shm_info_.readOnly  = False;

    if (!XShmAttach(dpy_, &shm_info_)) { free_xshm(); return false; }
    shm_attached_ = true;
...
void X11Window::free_xshm() noexcept {
    ...
    if (dst_pic_)      { XRenderFreePicture(dpy_, dst_pic_); dst_pic_ = 0; }
    if (src_pic_)      { XRenderFreePicture(dpy_, src_pic_); src_pic_ = 0; }
    if (render_pix_)   { XFreePixmap(dpy_, render_pix_);     render_pix_ = 0; }
    if (shm_attached_) { XShmDetach(dpy_, &shm_info_);       shm_attached_ = false; }
    if (shm_info_.shmaddr && shm_info_.shmaddr != reinterpret_cast<char*>(-1)) {
        shmdt(shm_info_.shmaddr);
        shm_info_.shmaddr = nullptr;
    }
    if (ximage_) { XDestroyImage(ximage_); ximage_ = nullptr; }
    shm_w_ = shm_h_ = 0;
}
...
void X11Window::handle_event(const XEvent& ev) noexcept {
    ...
    case ConfigureNotify: {
        ...
        if (c.width > 0)  window_w_ = static_cast<u32>(c.width);
        if (c.height > 0) window_h_ = static_cast<u32>(c.height);
        break;
    }
    case ClientMessage:
        if (ev.xclient.format == 32 &&
            static_cast<Atom>(ev.xclient.data.l[0]) == wm_delete_) {
            closed_ = true;
        }
        break;
    case KeyPress:
    ...
    case ButtonRelease: {
        ...
        case Button1: input_push_mouse_button(0, down); break;
        case Button2: input_push_mouse_button(2, down); break;
        case Button3: input_push_mouse_button(1, down); break;
        case Button4: if (down) input_push_mouse_wheel( 1.f); break;
        case Button5: if (down) input_push_mouse_wheel(-1.f); break;
        default: break;
        }
        break;
    }
...
void X11Window::apply_blit(const render::Framebuffer& /*fb*/) noexcept {
    if (!ximage_ || !src_pic_ || !dst_pic_) return;
    ...
    XShmPutImage(dpy_, render_pix_, gc_, ximage_,
                 0, 0, 0, 0,
                 render_w_, render_h_, False);

    // Compute the affine for XRender: src extent / dst extent gives the
    // scale factor; we encode as XTransform fixed-point (16.16).
    auto fix = [](double v) -> XFixed {
        return static_cast<XFixed>(v * 65536.0);
    };
    ...
    XRenderComposite(dpy_, PictOpSrc,
                     src_pic_, None, dst_pic_,
                     0, 0,
                     0, 0,
                     r.x, r.y, r.w, r.h);
    XFlush(dpy_);
...
void X11Window::present(const render::Framebuffer& fb) {
    ...
    if (fb.width != shm_w_ || fb.height != shm_h_) {
        render_w_ = fb.width;
        render_h_ = fb.height;
        if (!allocate_xshm(render_w_, render_h_)) return;
    }
