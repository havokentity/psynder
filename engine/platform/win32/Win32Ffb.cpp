...
bool Win32Ffb::acquire_first_wheel(HWND hwnd) noexcept {
    ...
    HRESULT hr = ::DirectInput8Create(::GetModuleHandleW(nullptr),
                                      ...
                                      nullptr);
    if (!psy_hr_ok(hr, "DirectInput8Create"))
        return false;

    DIDEVICEINSTANCEW didi{};
    didi.dwSize = sizeof(DIDEVICEINSTANCEW);
    hr = dinput_->EnumDevices(DI8DEVCLASS_GAMECTRL,
                              &enum_first_wheel,
                              &didi,
    ...
    if (!psy_hr_ok(dinput_->CreateDevice(didi.guidInstance, device_.GetAddressOf(), nullptr),
                   "IDirectInput8::CreateDevice")) {
        dinput_.Reset();
        return false;
    }

    if (!psy_hr_ok(device_->SetDataFormat(&c_dfDIJoystick2), "IDirectInputDevice8::SetDataFormat")) {
        shutdown();
        return false;
    }
...
bool Win32Ffb::set_constant_force(LONG magnitude, DWORD direction) noexcept {
    ...
    if (!constant_) {
        HRESULT hr =
            device_->CreateEffect(GUID_ConstantForce, &eff, constant_.GetAddressOf(), nullptr);
        if (!psy_hr_ok(hr, "CreateEffect(ConstantForce)"))
            return false;
    ...
    return psy_hr_ok(constant_->SetParameters(&eff,
...
bool Win32Ffb::set_spring(LONG coefficient) noexcept {
    ...
    if (!spring_) {
        HRESULT hr = device_->CreateEffect(GUID_Spring, &eff, spring_.GetAddressOf(), nullptr);
        if (!psy_hr_ok(hr, "CreateEffect(Spring)"))
            return false;
        return psy_hr_ok(spring_->Start(1, 0), "Start(Spring)");
    }
    return psy_hr_ok(spring_->SetParameters(&eff, DIEP_TYPESPECIFICPARAMS | DIEP_START),
