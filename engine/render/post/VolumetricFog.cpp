...
PSY_FORCEINLINE f32 z_to_slice(f32 z, f32 near_z, f32 far_z) noexcept {
    if (z <= near_z)
        return 0.0f;
    if (z >= far_z)
        return static_cast<f32>(kFroxelD);
...
math::Vec3 froxel_centre(const FogScene& s, u32 fx, u32 fy, u32 fz) noexcept {
    ...
    return math::Vec3{
        s.camera_position.x + s.camera_forward.x * z + s.camera_right.x * ndc_x * half_w +
            s.camera_up.x * ndc_y * half_h,
        s.camera_position.y + s.camera_forward.y * z + s.camera_right.y * ndc_x * half_w +
            s.camera_up.y * ndc_y * half_h,
        s.camera_position.z + s.camera_forward.z * z + s.camera_right.z * ndc_x * half_w +
...
math::Vec3 in_scatter_at(const math::Vec3& world_pos, const FogScene& s, f32 light_step_size) noexcept {
...
void populate_fog_grid(FroxelGrid& grid, const FogScene& scene) {
    ...
    const f32 mid_step = (scene.far_z - scene.near_z) / static_cast<f32>(kFroxelD);
...
void apply_volumetric_fog(Framebuffer& hdr,
    ...
    const usize pitch_pix = hdr.pitch ? (hdr.pitch / sizeof(detail::HdrPixel)) : hdr.width;
    ...
    for (u32 y = 0; y < h; ++y) {
        for (u32 x = 0; x < w; ++x) {
            ...
            if (depth_linear) {
                ...
                if (z_back <= 0.0f || z_back > scene.far_z)
                    z_back = scene.far_z;
            }
            ...
            const u32 ix =
                std::min<u32>(static_cast<u32>((static_cast<f32>(x) + 0.5f) / static_cast<f32>(w) *
                                               static_cast<f32>(kFroxelW)),
                              kFroxelW - 1u);
            const u32 iy =
                std::min<u32>(static_cast<u32>((static_cast<f32>(y) + 0.5f) / static_cast<f32>(h) *
                                               static_cast<f32>(kFroxelH)),
                              kFroxelH - 1u);

            const u32 iz_end_capped = std::min<u32>(kFroxelD - 1u, static_cast<u32>(std::ceil(s_end)));
            ...
            for (u32 sz = 0; sz <= iz_end_capped; ++sz) {
                ...
                if (thick_far <= thick_near)
                    continue;
                const f32 dz = thick_far - thick_near;

                const Froxel& cell = grid.cells[FroxelGrid::index_of(ix, iy, sz)];
                ...
                if (trans < 1e-3f)
                    break;  // visually opaque, early-out
            }

            // Composite: out_colour = fb * trans + scatter * intensity.
            detail::HdrPixel& p = hdr_pix[static_cast<usize>(y) * pitch_pix + x];
