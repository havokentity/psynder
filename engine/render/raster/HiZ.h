...
struct HiZTile {
    ...
    void clear() noexcept {
        for (u32 i = 0; i < kCells; ++i) max_z[i] = 1.0f;
    }
    ...
    PSY_FORCEINLINE bool any_cell_passes(i32 x0, i32 y0, i32 x1, i32 y1,
                                         f32 test_z) const noexcept {
        // Convert pixel rect → cell rect.
        if (x0 < 0) x0 = 0;
        if (y0 < 0) y0 = 0;
        if (x1 > static_cast<i32>(TILE_W)) x1 = static_cast<i32>(TILE_W);
        if (y1 > static_cast<i32>(TILE_H)) y1 = static_cast<i32>(TILE_H);
        ...
        for (u32 cy = cy0; cy < cy1 && cy < kRows; ++cy) {
            for (u32 cx = cx0; cx < cx1 && cx < kCols; ++cx) {
                if (max_z[cy * kCols + cx] > test_z) return true;
            }
        }
        return false;
    }
    ...
    PSY_FORCEINLINE void update_cell(u32 cell, f32 new_max_z) noexcept {
        if (new_max_z < max_z[cell]) max_z[cell] = new_max_z;
    }
};
