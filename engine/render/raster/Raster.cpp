...
struct FrameArena {
    ...
    template <class T> T* alloc_array(usize n) noexcept {
...
void Rasterizer::submit(const DrawItem& draw) {
    auto& fs = frame_state();
    if (!fs.in_frame || fs.draw_count >= FrameState::kMaxDraws) return;
    fs.draw_items[fs.draw_count++] = draw;
}

void Rasterizer::end_frame() {
    auto& fs = frame_state();
    if (!fs.in_frame) return;
    fs.in_frame = false;

    if (fs.view.target.width == 0 || fs.view.target.height == 0) return;
    ...
    for (u32 di = 0; di < fs.draw_count; ++di) {
        ...
        if (!cp) { fs.draw_cmds[di] = DrawCmd{}; continue; }
        ...
        if (!tris) { fs.draw_cmds[di] = DrawCmd{}; continue; }
        u32 valid_tris = 0;
        for (u32 ti = 0; ti < tri_count; ++ti) {
            ...
            setup_triangle(cp[i0], cp[i1], cp[i2],
                           d.vertices[i0].uv, d.vertices[i1].uv, d.vertices[i2].uv,
                           d.vertices[i0].color, d.vertices[i1].color, d.vertices[i2].color,
                           fs.view.target.width, fs.view.target.height,
                           tris[ti]);
            if (tris[ti].valid) ++valid_tris;
        }
    ...
    if (!grid.tile_offset || !grid.tile_count) return;
    ...
    grid.entries_used     = 0;
    grid.entries          = total_entries
                                ? fs.arena.alloc_array<BinEntry>(total_entries)
                                : nullptr;
    if (total_entries && !grid.entries) return;
    ...
    for (u32 ty = 0; ty < grid.rows; ++ty) {
        for (u32 tx = 0; tx < grid.cols; ++tx) {
            fn(fs.view.target, fs.draw_cmds, fs.draw_count, grid,
               /*tex*/ nullptr, tx, ty, affine_mode);
        }
    }
}
...
void clear_framebuffer(Framebuffer& fb, u32 rgba) noexcept {
    ...
    if (fb.depth) {
        ...
        for (usize i = 0; i < dcount; ++i) fb.depth[i] = packed_far;
    }
}
