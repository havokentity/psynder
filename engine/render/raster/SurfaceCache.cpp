...
// ─── Lookup helpers ──────────────────────────────────────────────────────
u32 SurfaceCache::lookup_slot(u32 surface_id, u32 lightmap_version,
    ...
    for (u32 i = 0; i < kMaxEntries; ++i) {
        ...
        if (e.byte_size == 0) continue;   // streak-only slot, not a payload
...
u32 SurfaceCache::lookup_streak_slot(u32 surface_id) const noexcept {
    for (u32 i = 0; i < kMaxEntries; ++i) {
        const SurfaceCacheEntry& e = entries_[i];
        if (!e.occupied) continue;
        if (e.surface_id == surface_id) return i;
    }
    return kInvalid;
}

u32 SurfaceCache::find(u32 surface_id, u32 lightmap_version,
                       u32 mip_level) noexcept {
...
u32 SurfaceCache::acquire(u32 surface_id, u32 lightmap_version,
                          u32 mip_level, u32 width, u32 height,
                          u32 byte_size) noexcept {
    ...
    if (byte_size > kSlabBytes) return kInvalid;

    // Evict if needed.
    if (slab_used_ + byte_size > kSlabBytes) {
        if (!evict_for(byte_size)) return kInvalid;
    }
    ...
    if (slot == kInvalid) {
        for (u32 i = 0; i < kMaxEntries; ++i) {
            if (!entries_[i].occupied) { slot = i; break; }
        }
    }
    if (slot == kInvalid) {
        // No empty slot — evict the LRU tail.
        if (lru_tail_ == kInvalid) return kInvalid;
        slot = lru_tail_;
        lru_detach(slot);
...
void SurfaceCache::lru_detach(u32 slot) noexcept {
    SurfaceCacheEntry& e = entries_[slot];
    if (e.lru_prev != kInvalid) entries_[e.lru_prev].lru_next = e.lru_next;
    if (e.lru_next != kInvalid) entries_[e.lru_next].lru_prev = e.lru_prev;
    if (lru_head_ == slot) lru_head_ = e.lru_next;
    if (lru_tail_ == slot) lru_tail_ = e.lru_prev;
    e.lru_prev = kInvalid;
    e.lru_next = kInvalid;
}
...
void SurfaceCache::lru_push_front(u32 slot) noexcept {
    ...
    if (lru_head_ != kInvalid) entries_[lru_head_].lru_prev = slot;
    lru_head_ = slot;
...
bool SurfaceCache::evict_for(u32 bytes_needed) noexcept {
    ...
    // already over budget).
    while (slab_used_ + bytes_needed > kSlabBytes) {
        if (lru_tail_ == kInvalid) return false;
        u32 victim = lru_tail_;
...
const SurfaceCacheEntry* SurfaceCache::entry(u32 slot) const noexcept {
    if (slot >= kMaxEntries) return nullptr;
    if (!entries_[slot].occupied) return nullptr;
...
ShadingPath classify_surface(const SurfaceDesc& s) noexcept {
    ...
    if (cv.r_force) {
        ...
        if (v == 1) return ShadingPath::OnTheFly;
        if (v == 2) return ShadingPath::SurfaceCached;
    }
    auto& cache = SurfaceCache::Get();
    const bool eligible = (s.flags & draw_flags::kEligibleMask) ==
                          draw_flags::kEligibleMask;
    if (!eligible) {
    ...
    return streak >= SurfaceCache::kHysteresis
        ? ShadingPath::SurfaceCached
        : ShadingPath::OnTheFly;
}
