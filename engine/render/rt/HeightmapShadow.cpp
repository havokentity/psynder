...
PSY_FORCEINLINE
bool clip_y_slab(f32 oy, f32 dy, f32 y_min, f32 y_max, f32 t_in, f32 t_out) noexcept {
    (void)y_min;
...
bool trace_heightmap_shadow(const Heightmap& hm, const Ray& ray, u32 max_steps) noexcept {
    ...
    auto world_to_cell_x = [&](f32 wx) noexcept { return (wx - hm.origin_xz.x) / hm.cell_size; };
    auto world_to_cell_z = [&](f32 wz) noexcept { return (wz - hm.origin_xz.y) / hm.cell_size; };

    // ─── Initial t window ────────────────────────────────────────────────
    f32 t_in = ray.t_min;
    f32 t_out = ray.t_max;
    if (!clip_y_slab(ray.origin.y, ray.direction.y, hm.y_min, hm.y_max, t_in, t_out)) {
        return false;
    }
    if (t_out < t_in)
        return false;
    ...
    for (u32 s = 0; s < max_steps && t <= t_out; ++s) {
        ...
        if (cx < 0.0f || cz < 0.0f || cx > static_cast<f32>(hm.width - 1) ||
