...
void Archetype::init(u32 id, std::span<const ComponentId> sorted_components) {
    ...
    const usize version_bytes = align_up(sizeof(u32) * components_.size(), kColumnAlign);
...
void Archetype::bump_version(Chunk* c, u32 col_index) noexcept {
    auto* v = reinterpret_cast<u32*>(c->at(c->header.version_stamps_offset));
    ++v[col_index];
    // Mark the corresponding dirty-mask bucket.
    constexpr u32 kBitsPerBucket = 4;
    const u32 bucket = col_index / kBitsPerBucket;
    if (bucket < 64)
        c->header.dirty_mask |= (u64{1} << bucket);
}

u32 Archetype::swap_pop(u32 chunk_index, u32 row, std::span<const u32> /*entity_per_row_for_chunk*/) {
    ...
    while (last_chunk_idx > chunk_index && chunks_[last_chunk_idx]->header.row_count == 0) {
        --last_chunk_idx;
    }
    ...
    if (!same_slot) {
        ...
        for (usize i = 0; i < components_.size(); ++i) {
            std::byte* dst = c->at(column_offsets_[i]) + static_cast<usize>(row) * column_sizes_[i];
            std::byte* src =
                last_chunk->at(column_offsets_[i]) + static_cast<usize>(last_row) * column_sizes_[i];
