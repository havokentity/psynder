...
ISpatialIndex* resolve(SpatialBackend backend) noexcept {
    switch (backend) {
        case SpatialBackend::SweepAndPrune:
            return sap_backend();
        case SpatialBackend::Bvh:
            return bvh_backend();
        case SpatialBackend::HashedGrid:
            return grid_backend();
...
PSY_FORCEINLINE SpatialBackend default_for_kind(QueryKind kind) noexcept {
    switch (kind) {
        case QueryKind::Raycast:
            return SpatialBackend::Bvh;
        case QueryKind::FrustumCull:
            return SpatialBackend::Bvh;
        case QueryKind::AabbOverlap:
            return SpatialBackend::Bvh;
        case QueryKind::Broadphase:
            return SpatialBackend::SweepAndPrune;
        case QueryKind::NearestNeighbour:
            return SpatialBackend::HashedGrid;
    }
...
SpatialBackend route_query(QueryKind kind, RegionId region) noexcept {
    for (const auto& o : overrides()) {
        if (o.kind == kind && o.region == region)
            return o.backend;
    }
    ...
    if (region != kGlobalRegion) {
        for (const auto& o : overrides()) {
            if (o.kind == kind && o.region == kGlobalRegion)
                return o.backend;
        }
    }
...
void set_region_override(QueryKind kind, RegionId region, SpatialBackend backend) noexcept {
    auto& v = overrides();
    auto it = std::find_if(v.begin(), v.end(), [&](const Override& o) noexcept {
