                    ...
                    const math::Aabb& node_bounds)
{
    ...
    for (u32 axis = 0; axis < 3; ++axis) {
        ...
        for (u32 i = 0; i < count; ++i) {
            ...
            if (b >= kBvhSahBuckets) b = kBvhSahBuckets - 1;
        ...
        for (u32 b = 0; b + 1 < kBvhSahBuckets; ++b) {
            if (left_counts[b] == 0 || right_counts[b + 1] == 0) continue;
            const f32 sa_l = aabb_surface_area(left_bounds[b]);
            const f32 sa_r = aabb_surface_area(right_bounds[b + 1]);
            const f32 cost = 1.0f
                + (sa_l * static_cast<f32>(left_counts[b])
                   + sa_r * static_cast<f32>(right_counts[b + 1])) / node_sa;
            if (cost < best_cost) {
                best_cost         = cost;
                best_axis         = axis;
                best_split_bucket = b;
    ...
    if (best_cost >= leaf_cost) return out;
    ...
    auto mid_iter = std::partition(
        prim_idx.begin() + first,
        prim_idx.begin() + first + count,
        [&](u32 pi) noexcept { return bucket_of(pi) <= best_split_bucket; });

    const u32 mid = static_cast<u32>(
        mid_iter - (prim_idx.begin() + first)) + first;

    if (mid == first || mid == first + count) return out;
...
u32 build_recursive(BvhState& s, u32 first, u32 count,
                    const math::Aabb& node_bounds,
                    const math::Aabb& centroid_bounds,
                    u32 parent)
{
    ...
    const auto split = best_split(s.prim_indices, s.prims,
                                  first, count, centroid_bounds, node_bounds);
    if (!split.found) {
        s.nodes[idx].left       = 0xFFFFFFFFu;
        s.nodes[idx].right      = 0xFFFFFFFFu;
        s.nodes[idx].first_prim = first;
    ...
    const u32 left_idx = build_recursive(s, first, split.mid - first,
                                          split.left_bounds, left_centroids, idx);
    const u32 right_idx = build_recursive(s, split.mid, first + count - split.mid,
                                           split.right_bounds, right_centroids, idx);
...
void rebuild(BvhState& s) noexcept {
    ...
    for (u32 i = 0; i < s.prims.size(); ++i) {
        if (s.prims[i].entity_index == 0xFFFFFFFFu) continue;
    ...
    build_recursive(s, 0, static_cast<u32>(s.prim_indices.size()),
                    node_bounds, centroid_bounds, 0xFFFFFFFFu);
...
public:
    ...
    void update(SpatialKey key, const math::Aabb& bounds) override {
        ...
        if (slot >= g_bvh.prims.size()) return;
        if (g_bvh.prims[slot].entity_index == 0xFFFFFFFFu) return;
    ...
    void query_aabb(const math::Aabb& q, std::span<u32> out_entities) const override {
        ...
        if (g_bvh.nodes.empty() || out_entities.empty()) return;
...
// ─── Exports ────────────────────────────────────────────────────────────
BvhState& bvh_state() noexcept { return g_bvh; }
...
BvhRefitStats bvh_refit() noexcept {
    ...
    stats.sah_cost_as_built  = g_bvh.sah_cost_as_built;
    stats.should_async_rebuild =
        g_bvh.sah_cost_as_built > 0.0f &&
