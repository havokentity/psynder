...
void diag_push(std::string_view text) noexcept {
    ...
    if (g_diag.count < kDiagRingCapacity) ++g_diag.count;
}
...
void strip_push(f32 ms) noexcept {
    ...
    if (g_strip.count < kStripSamples) ++g_strip.count;
}
...
void format_frame_stats(f32 frame_ms,
                        char* out_ms,
                        usize out_ms_cap,
                        char* out_fps,
                        usize out_fps_cap) noexcept {
...
DebugHudMode debug_hud_mode() noexcept { return g_mode; }
void set_debug_hud_mode(DebugHudMode mode) noexcept { g_mode = mode; }

void push_diag_line(std::string_view line) noexcept { diag_push(line); }
...
usize draw_debug_hud(render::Framebuffer&  fb,
    ...
    if (g_mode == DebugHudMode::Off) return 0;
    if (!d::framebuffer_drawable(fb)) return 0;
    ...
    for (u32 y = 0; y < y1; ++y) {
        const u32* row = reinterpret_cast<const u32*>(
            fb.pixels + static_cast<usize>(fb.pitch) * y);
        for (u32 x = 0; x < x1; ++x) {
            ...
            if (sidx < snapshot_cap) snapshot[sidx] = row[x];
        }
    }
    ...
    char ms_buf[32]{};
    char fps_buf[24]{};
    format_frame_stats(stats.frame_ms, ms_buf, sizeof(ms_buf),
                                       fps_buf, sizeof(fps_buf));
    label(math::Vec2{ panel_origin.x + 4.0f, panel_origin.y + 4.0f },
          std::string_view{ ms_buf }, kColourLabel);

    const std::string_view fps_view{ fps_buf };
    const u32 fps_px = d::text_width(fps_view);
    const f32 fps_x  = panel_origin.x + panel_size.x - 4.0f
                     - static_cast<f32>(fps_px);
    label(math::Vec2{ fps_x, panel_origin.y + 4.0f },
          fps_view, kColourFps);
    ...
    label(math::Vec2{ panel_origin.x + 4.0f,
                      panel_origin.y + 4.0f + kLineHeight },
          std::string_view{ meta_buf }, kColourLabel);
    ...
    if (g_mode == DebugHudMode::Compact) {
        ...
        graph(chart_origin, chart_size, stats.frame_ms,
              /*max_ms*/ 33.3f, std::string_view{});
    } else {
        ...
        graph(chart_origin, chart_size, stats.frame_ms,
              /*max_ms*/ 33.3f, std::string_view{ "frame ms" });
        ...
        const u32 start =
            (g_diag.head + kDiagRingCapacity - visible) % kDiagRingCapacity;
        for (u32 i = 0; i < visible; ++i) {
            const DiagLine& dl = g_diag.lines[(start + i) % kDiagRingCapacity];
            label(math::Vec2{ panel_origin.x + 4.0f,
                              log_y + static_cast<f32>(i) * kLineHeight },
    ...
    for (u32 y = 0; y < y1; ++y) {
        ...
        for (u32 x = 0; x < x1; ++x) {
            const usize sidx = static_cast<usize>(y) * x1 + x;
            if (sidx >= snapshot_cap) continue;
            if (row[x] != snapshot[sidx]) ++written;
        }
    }
    return written;
}
