...
[[nodiscard]] ReadOutcome read_pair(detail::Document& doc) {
    ...
    if (!rml_blob.data || rml_blob.bytes == 0) {
        ...
        PSY_LOG_INFO("rml: VFS returned no bytes for '{}' (VFS stub)", doc.rml_virtual_path);
        return ReadOutcome::NoSource;
    }
    std::string_view rml_src(reinterpret_cast<const char*>(rml_blob.data), rml_blob.bytes);
    ...
    if (!doc.rcss_virtual_path.empty()) {
        ...
        if (css_blob.data && css_blob.bytes > 0) {
            rcss_src = std::string_view(reinterpret_cast<const char*>(css_blob.data), css_blob.bytes);
        }
    }

    ParsedPair pp =
        parse_pair_from_sources(rml_src, rcss_src, doc.rml_virtual_path, doc.rcss_virtual_path);
    if (!pp.ok)
...
void update(f32 /*dt*/) {
    ...
    for (auto& [name, doc] : s.documents) {
        ...
        if (outcome == ReadOutcome::Ok) {
            ...
            PSY_LOG_INFO("rml: hot-reloaded '{}' (gen {})", name, doc.reload_generation);
...
bool inject_source(std::string_view name, std::string_view rml_src, std::string_view rcss_src) {
    ...
    if (!res.ok) {
        PSY_LOG_WARN(
            "rml: test_only inject_source('{}') rml parse failed "
            "line {}: {}",
            std::string(name),
            res.error_line,
            res.error);
    ...
    if (!rcss_src.empty()) {
        ...
        if (!cssr.ok) {
            PSY_LOG_WARN(
                "rml: test_only inject_source('{}') rcss parse failed "
                "line {}: {}",
                std::string(name),
                cssr.error_line,
                cssr.error);
...
void render_layout(std::string_view name, math::Vec2 viewport, std::vector<detail::LayoutBox>& out) {
