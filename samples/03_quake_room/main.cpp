...
void emit_quad(World& w,
               ...
               math::Vec3 normal, u32 color, u32 material_id) {
...
void build_world(World& w) {
    ...
    emit_quad(w,
        ...
        {0, 1, 0}, kColRoomAFloor, kMatFloor);
    // Ceiling
    emit_quad(w,
...
void emit_leaf_faces(const world::bsp::BspLeaf& leaf, void* user) {
    ...
    if (leaf.face_count == 0) return;
    const usize face_lo = leaf.first_face;
    const usize face_hi = std::min<usize>(face_lo + leaf.face_count,
                                          w.map.faces.size());
...
int main(int argc, char** argv) {
    ...
    PSY_LOG_INFO("sample_03: world built — {} faces, {} leaves, {} verts",
                 w.map.faces.size(), w.map.leaves.size(), w.verts.size());
    ...
    while (!window->should_close()) {
        ...
        const f32 dt = (args.smoke_frames > 0)
                          ? 1.0f / 60.0f
                          : std::min(0.1f, static_cast<f32>(
                                platform::Clock::seconds(now - last_ticks)));
        last_ticks = now;
        ...
        if (input->key_down(platform::KeyCode::W)) move = math::add(move, fwd_xz);
        if (input->key_down(platform::KeyCode::S)) move = math::sub(move, fwd_xz);
        if (input->key_down(platform::KeyCode::D)) move = math::add(move, right_xz);
        if (input->key_down(platform::KeyCode::A)) move = math::sub(move, right_xz);
        ...
        view.view       = math::look_at_rh(
                              cam.pos,
                              math::add(cam.pos, fwd),
                              math::Vec3{0, 1, 0});
        view.projection = math::perspective_rh(
                              70.0f * math::kDegToRad,
        ...
        world::bsp::walk_visible_leaves(w.map, cam.pos,
                                        &emit_leaf_faces, &ctx);
        ...
        if (args.smoke_frames > 0) {
            PSY_LOG_INFO("sample_03: frame {} — eye ({:.2f},{:.2f},{:.2f}) "
                         "submitted {} draws",
                         frame, cam.pos.x, cam.pos.y, cam.pos.z, ctx.draw_count);
        }

        ++frame;
        if (args.smoke_frames > 0 && frame >= args.smoke_frames) {
            PSY_LOG_INFO("sample_03: smoke target reached ({}); exiting",
                         args.smoke_frames);
            break;
        }
    }
    ...
    if (!args.capture_out.empty()) {
        const bool ok = samples::write_png_rgba8_framebuffer(
            ...
            fb.width, fb.height);
        if (!ok) {
            PSY_LOG_ERROR("sample_03: failed to write capture to {}",
                          args.capture_out);
