...
math::Mat4 mat4_trs(math::Vec3 t, f32 scale) {
    math::Mat4 m{};
    m.m[0]  = scale; m.m[1]  = 0;     m.m[2]  = 0;     m.m[3]  = 0;
    m.m[4]  = 0;     m.m[5]  = scale; m.m[6]  = 0;     m.m[7]  = 0;
    m.m[8]  = 0;     m.m[9]  = 0;     m.m[10] = scale; m.m[11] = 0;
...
void orbit_lights(f32 t_seconds, std::array<Light, kNumLights>& lights) {
    ...
    for (u32 i = 0; i < kNumLights; ++i) {
        ...
        lights[i].position = {
            std::cos(a) * radii[i], heights[i], std::sin(a) * radii[i]};
        lights[i].r = cols[i][0];
        lights[i].g = cols[i][1];
        lights[i].b = cols[i][2];
...
void upsample_bilinear(const u32* src, u32 sw, u32 sh,
    ...
    for (u32 y = 0; y < dh; ++y) {
        ...
        for (u32 x = 0; x < dw; ++x) {
            ...
            dst[static_cast<usize>(y) * dw + x] =
                pack_rgba8(clamp_u8(r), clamp_u8(g), clamp_u8(b));
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    ground_blas.build(ground_tris.data(),
                      static_cast<u32>(ground_tris.size()));
    ...
    for (u32 i = 0; i < kNumCubes; ++i) {
        ...
        insts[i].transform = mat4_trs(cube_instances[i].center,
                                       cube_instances[i].size);
    }
    ...
    const f32 aspect =
        static_cast<f32>(kFbW) / static_cast<f32>(kFbH);
    ...
    while (!window->should_close()) {
        ...
        for (u32 y = 0; y < kShadowH; ++y) {
            for (u32 x = 0; x < kShadowW; ++x) {
                const f32 nx = (static_cast<f32>(x) + 0.5f)
                              / static_cast<f32>(kShadowW);
                const f32 ny = (static_cast<f32>(y) + 0.5f)
                              / static_cast<f32>(kShadowH);
                const render::rt::Ray ray = primary_ray(cam, nx, ny);
                const render::rt::Hit  h  = tlas.intersect(ray);

                const usize idx =
                    static_cast<usize>(y) * kShadowW + x;
                PixelHit ph{};
                ph.hit = h.hit;
                if (!h.hit) {
        ...
        std::vector<f32> accum(
            static_cast<usize>(kShadowW) * kShadowH * 3, 0.0f);
        ...
        for (u32 li = 0; li < kNumLights; ++li) {
            ...
            auto dispatch = [&]() {
                ...
                for (u32 i = 0; i < 8; ++i) {
                    if (!pkt_real[i]) continue;
                    const usize px      = pkt_idx[i];
                    if (pkt.occluded[i]) continue;
            ...
            for (usize px = 0, n = hits.size(); px < n; ++px) {
                ...
                if (math::dot(ph.normal, ld) <= 0.0f) continue;
                ...
                ++in_pkt;
                if (in_pkt == 8) dispatch();
            }
            if (in_pkt > 0) dispatch();
        }
        ...
        for (u32 y = 0; y < kShadowH; ++y) {
            for (u32 x = 0; x < kShadowW; ++x) {
                ...
                if (!hits[idx].hit) continue;   // sky already written
    ...
    if (!args.capture_out.empty()) {
        const bool ok = samples::write_png_rgba8_framebuffer(
            args.capture_out.c_str(), final_pixels.data(),
            fb.width, fb.height);
        if (!ok) {
            PSY_LOG_ERROR("sample_05: failed to write capture to {}",
                          args.capture_out);
