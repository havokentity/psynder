...
void bench_step(Vehicle& v, f32 dt) {
    ...
    auto out = kernels::kernel_drivetrain_step(
        v.drive, v.throttle, v.brake, v.clutch, v.gear, om_l, om_r, v.engine_rpm);
    ...
    for (Wheel& w : v.wheels) {
        ...
        f32 slip_lat  = std::atan2(0.5f, std::max(std::fabs(v.forward_v), 1.0f))
                      - w.steer;
        auto t = kernels::kernel_pacejka_combined(slip_long, slip_lat,
                                                  w.load_n, 1.0f, v.tire);
        total_fx += t.Fx;
        total_fy += t.Fy;
    ...
    // Aero on the chassis.
    auto aero = kernels::kernel_aero_force(
        math::Vec3{v.forward_v, 0, 0}, math::Vec3{0, -1, 0},
        0.30f, 2.2f, 1.5f, 1.0f);
    ...
    if (v.forward_v < 5.0f) v.forward_v = 5.0f;
    if (v.forward_v > 80.0f) v.forward_v = 80.0f;
    (void)total_fy;
}
...
int main(int argc, char** argv) {
    ...
    for (u32 i = 0; i < kWarm; ++i) bench_step(v, 1.0f / 120.0f);

    std::vector<f64> samples;
    samples.reserve(kSteps);
    for (u32 i = 0; i < kSteps; ++i) samples.push_back(bench_one_step(v));
    ...
    if (!std::isfinite(v.engine_rpm) || !std::isfinite(v.forward_v)) return 1;
    return 0;
}
