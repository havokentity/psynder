...
BenchResult bench_raster_tile_clear(u64 iters) {
    ...
    for (u64 i = 0; i < iters; ++i) {
        render::raster::clear_framebuffer(fb, static_cast<u32>(i * 2654435761u));
    }
    do_not_optimize(tile_pixels);
...
BenchResult bench_raster_pipeline_tile(u32 tile_size, u64 iters) {
    ...
    const f64 pixels_total = static_cast<f64>(iters) * static_cast<f64>(W) * static_cast<f64>(H);
...
std::vector<std::pair<std::string, f64>> parse_baseline(const std::string& path) {
    ...
    while (true) {
        ...
        if (ns_key == std::string::npos)
            break;
        const f64 val =
            std::strtod(body.c_str() + ns_key + std::string_view("\"ns_per_iter\":").size(), nullptr);
...
Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (s == "--smoke")
            ...
            a.iters = std::strtoull(std::string(s.substr(std::string_view("--iters=").size())).c_str(),
                                    nullptr,
                                    10);
            ...
            a.regression_pct =
                std::strtod(std::string(s.substr(std::string_view("--regression-pct=").size())).c_str(),
                            nullptr);
    }
    return a;
}
...
int main(int argc, char** argv) {
    ...
    const u64 raster_iters = args.smoke ? 8 : (args.iters ? args.iters : 50'000);
    ...
    const u64 pipeline_iters = args.smoke ? 1 : (args.iters ? args.iters : 200);
    const u64 physics_iters = args.smoke ? 4 : (args.iters ? args.iters : 5'000);
    ...
    if (!args.baseline.empty() && !args.smoke) {
        ...
        if (baseline.empty()) {
            PSY_LOG_WARN("bench: baseline {} empty or unparseable; not gating", args.baseline);
            return EXIT_SUCCESS;
        }
        ...
        for (const BenchResult& r : results) {
            for (const auto& [bn, bv] : baseline) {
                ...
                if (pct > budget) {
                    ...
                    PSY_LOG_INFO("bench: {} delta {:+.2f}% vs baseline {:.2f}ns", r.name, pct, bv);
                }
                break;
            }
        }
