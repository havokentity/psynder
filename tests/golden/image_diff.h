...
struct CompareResult {
    ...
    f64 mismatch_pct() const noexcept {
        return total_pixels == 0
                   ? 0.0
                   : 100.0 * static_cast<f64>(mismatch_count) / static_cast<f64>(total_pixels);
    }
};
...
inline CompareResult compare_images(const Image& a, const Image& b, u32 channel_eps = 8) noexcept {
    CompareResult r{};
