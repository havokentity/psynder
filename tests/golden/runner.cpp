...
bool write_ppm(const fs::path& path, const Image& img) {
    ...
    const bool ok = std::fwrite(img.rgb.data(), 1, img.rgb.size(), f) == img.rgb.size();
    std::fclose(f);
    return ok;
}
...
bool load_scene(const fs::path& path, Scene& out) {
    ...
    while (std::getline(in, line)) {
        ...
        if (op == "size" && tok.size() == 3) {
            ...
            i32 r, g, b;
            if (!parse_uint(tok[1], r) || !parse_uint(tok[2], g) || !parse_uint(tok[3], b))
                return false;
            ...
            Rect r{};
            i32 cr, cg, cb;
            if (!parse_uint(tok[1], r.x) || !parse_uint(tok[2], r.y) || !parse_uint(tok[3], r.w) ||
                !parse_uint(tok[4], r.h) || !parse_uint(tok[5], cr) || !parse_uint(tok[6], cg) ||
                !parse_uint(tok[7], cb))
                return false;
            ...
            if (!parse_float(tok[1], t.x0) || !parse_float(tok[2], t.y0) ||
                ...
                !parse_uint(tok[7], cr) || !parse_uint(tok[8], cg) || !parse_uint(tok[9], cb))
                return false;
            ...
            PSY_LOG_WARN("golden: unknown directive '{}' in {}", op, path.string());
            return false;
        }
    }
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--update")
            ...
            tolerance_pct =
                std::strtod(std::string(a.substr(std::string_view("--tolerance-pct=").size())).c_str(),
                            nullptr);
    }

    if (cases_dir.empty()) {
        if (auto found = find_relative_root("tests/golden/cases"))
            cases_dir = *found;
    }
    if (refs_dir.empty()) {
        if (auto found = find_relative_root("tests/golden/references"))
            refs_dir = *found;
    }

    if (cases_dir.empty() || !fs::exists(cases_dir)) {
        PSY_LOG_ERROR(
            "golden: cases directory not found "
    ...
    for (auto& e : fs::directory_iterator(cases_dir)) {
        if (!e.is_regular_file())
            continue;
        if (e.path().extension() == ".psyscene")
            cases.push_back(e.path());
    }
    std::sort(cases.begin(), cases.end());

    if (cases.empty()) {
        PSY_LOG_ERROR("golden: no .psyscene cases found under {}", cases_dir.string());
        return EXIT_FAILURE;
    }
    ...
    for (const fs::path& case_path : cases) {
        ...
        if (cr.mismatch_pct() > tolerance_pct) {
            ...
            const fs::path actual_path = fs::current_path() / (name + ".actual.ppm");
            write_ppm(actual_path, rendered);
            PSY_LOG_INFO("golden: wrote actual to {}", actual_path.string());
            ++failed;
        } else {
            PSY_LOG_INFO("golden: case '{}' OK — {:.4f}% mismatch", name, cr.mismatch_pct());
            ++passed;
        }
    }

    PSY_LOG_INFO("golden: {}/{} cases passed ({} failed)", passed, passed + failed, failed);
