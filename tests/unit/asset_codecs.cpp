...
asset::lmm::Mesh make_test_mesh(u32 vertex_count, u32 index_count, u16 submesh_count) {
    ...
    mesh.bbox_min[0]   = -1.0f; mesh.bbox_min[1] = -1.5f; mesh.bbox_min[2] = -2.0f;
    mesh.bbox_max[0]   =  1.0f; mesh.bbox_max[1] =  1.5f; mesh.bbox_max[2] =  2.0f;
    ...
    for (u16 s = 0; s < submesh_count; ++s) {
        ...
        sm.index_count   = (s == submesh_count - 1u)
                               ? (index_count - sm.index_start)
                               : chunk;
...
bool meshes_equal(const asset::lmm::Mesh& a, const asset::lmm::Mesh& b) {
    if (a.vertex_fmt    != b.vertex_fmt)    return false;
    if (a.vertex_stride != b.vertex_stride) return false;
    if (a.vertex_count  != b.vertex_count)  return false;
    if (a.index_count   != b.index_count)   return false;
    if (std::memcmp(a.bbox_min, b.bbox_min, sizeof(a.bbox_min)) != 0) return false;
    if (std::memcmp(a.bbox_max, b.bbox_max, sizeof(a.bbox_max)) != 0) return false;
    ...
    for (usize i = 0; i < a.submeshes.size(); ++i) {
        ...
        if (l.index_start != r.index_start) return false;
        if (l.index_count != r.index_count) return false;
        if (l.material_hash != r.material_hash) return false;
        if (l.reserved != r.reserved) return false;
    }
...
TEST_CASE("asset/codecs: .lmm writer ↔ reader round-trip (u16 indices)",
          "[asset][codecs][lmm]") {
...
TEST_CASE("asset/codecs: .lmm writer ↔ reader round-trip (u32 indices, wide mesh)",
    ...
    auto mesh = make_test_mesh(/*vertex_count=*/70'000, /*index_count=*/180,
                               /*submesh_count=*/3);
...
TEST_CASE("asset/codecs: .lmm reader rejects bad magic / version",
          "[asset][codecs][lmm]") {
...
TEST_CASE("asset/codecs: .lma writer ↔ reader round-trip (PCM_S16 mono)",
          "[asset][codecs][lma]") {
...
TEST_CASE("asset/codecs: writer rejects an inconsistent mesh",
          "[asset][codecs][lmm]") {
...
TEST_CASE("asset/codecs: writer rejects an audio buffer with bad pcm size",
          "[asset][codecs][lma]") {
    asset::lma::Audio audio;
    audio.sample_rate = 48000;
...
TEST_CASE("asset/codecs: Writer::write_entry stores compressed + uncompressed sizes",
    ...
    REQUIRE(w.write_entry("data/big.bin", payload.data(), payload.size(),
                          /*compress=*/true));
    // span overload exercises the alternative signature too.
    REQUIRE(w.write_entry("data/copy.bin", std::span<const u8>(payload),
                          /*compress=*/true));
    ...
    const auto* entries = reinterpret_cast<const asset::lmpak::LmpakEntry*>(
        archive.data() + hdr.entry_table_offset);
...
TEST_CASE("asset/codecs: Writer::write_entry uncompressed bypasses zstd",
          "[asset][codecs][lmpak]") {
    std::vector<u8> payload{0xDE, 0xAD, 0xBE, 0xEF};
    asset::lmpak::Writer w;
    REQUIRE(w.write_entry("misc/raw.bin", payload.data(), payload.size(),
                          /*compress=*/false));
    ...
    const auto* entry = reinterpret_cast<const asset::lmpak::LmpakEntry*>(
        archive.data() + hdr.entry_table_offset);
    ...
    REQUIRE(std::memcmp(archive.data() + entry->offset, payload.data(),
                        payload.size()) == 0);
}
