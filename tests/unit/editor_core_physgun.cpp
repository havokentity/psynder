...
TEST_CASE("physgun: ray-AABB intersect picks the nearest body", "[editor][physgun][pick]") {
    ...
    const physgun::PickResult r = physgun::pick(bodies, math::Vec3{0, 0, 0}, math::Vec3{0, 0, -1});
    REQUIRE(r.body_id == 1);
    REQUIRE(approx_eq(r.hit_distance, 2.5f));  // ray enters at -2.5 along -Z
}

TEST_CASE("physgun: ray that misses every AABB returns 0", "[editor][physgun][pick]") {
    std::vector<physgun::PickInput> bodies = {
        make_body(7, 10, 10, 10),
    };

    const physgun::PickResult r = physgun::pick(bodies, math::Vec3{0, 0, 0}, math::Vec3{1, 0, 0});
    REQUIRE(r.body_id == 0);
}
...
TEST_CASE("physgun: compute_pose places body at cursor + grab_distance", "[editor][physgun][drag]") {
    physgun::State s;
    s.body_id = 5;
    ...
    const physgun::PoseTarget t = physgun::compute_pose(s,
...
TEST_CASE("physgun: compute_pose scales multiplicatively", "[editor][physgun][drag]") {
    physgun::State s;
...
TEST_CASE("physgun: weld request midpoints the two body positions", "[editor][physgun][weld]") {
    const physgun::WeldRequest w =
        physgun::make_weld(11, 22, math::Vec3{4, 0, 0}, math::Vec3{-2, 0, 0});
    REQUIRE(w.body_a == 11);
    REQUIRE(w.body_b == 22);
