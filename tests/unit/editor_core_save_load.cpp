...
void build_sample_scene(detail::State& s) {
    ...
    // Constraints -----------------------------------------------------
    {
        ...
        g.add(constraints::make_axis(2,
                                     3,
                                     math::Vec3{1.75f, 0.75f, 0.0f},
                                     math::Vec3{0.0f, 0.0f, 1.0f},
                                     -math::kPi,
                                     math::kPi));
        g.add(constraints::make_rope(1,
                                     3,
                                     math::Vec3{0, 0, 0},
                                     math::Vec3{0, 0, 0},
                                     /*max_length=*/3.5f));
    ...
    for (u32 iz = 0; iz < s.heightfield.size_z; ++iz) {
        for (u32 ix = 0; ix < s.heightfield.size_x; ++ix) {
            s.heightfield.store(ix, iz, static_cast<f32>(ix) * 0.1f + static_cast<f32>(iz) * 0.01f);
        }
    }
...
TEST_CASE("save/load: .psylevel roundtrip preserves every section", "[editor][serial][roundtrip]") {
    detail::State src;
    ...
    for (usize ch = 0; ch < 4; ++ch) {
        REQUIRE(approx_eq(dst.splat.weights[idx][ch], src.splat.weights[idx][ch]));
    }
}

TEST_CASE("save/load: .psyc contraption roundtrip omits terrain", "[editor][serial][roundtrip]") {
    detail::State src;
    build_sample_scene(src);
...
TEST_CASE("save/load: decoder rejects truncated payload", "[editor][serial][roundtrip]") {
    detail::State src;
    build_sample_scene(src);
    std::vector<u8> blob;
    serial::encode_state(blob, src, /*with_terrain=*/true);
