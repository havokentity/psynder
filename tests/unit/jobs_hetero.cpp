...
TEST_CASE("submit_latency / submit_throughput both make forward progress", "[jobs][hetero]") {
...
TEST_CASE("hetero pool diagnostics are coherent across start/stop", "[jobs][hetero]") {
    auto& js = JobSystem::Get();
...
TEST_CASE(
    "mixed submit / submit_latency / submit_throughput keep both pools "
    "active",
    "[jobs][hetero]") {
    auto& js = JobSystem::Get();
    js.start();
...
TEST_CASE("dependency edge from latency parent to throughput child", "[jobs][hetero][graph]") {
    auto& js = JobSystem::Get();
    js.start();
