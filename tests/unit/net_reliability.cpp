...
TEST_CASE("net: reliability enqueue assigns monotonically increasing seqs", "[net][reliability]") {
    Reliability r;
...
TEST_CASE("net: ACK pipelining acks multiple in-flight frames in one reply",
    ...
    std::vector<u32> seqs;
    for (u32 i = 0; i < 8; ++i)
        seqs.push_back(sender.enqueue(payload, 0));
