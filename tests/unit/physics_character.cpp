...
TEST_CASE("Stance: water environment overrides all player intent",
    ...
    auto next = kernels::kernel_char_next_stance(
        kernels::CharStanceK::Stand,
        kernels::CharIntent{true, true, false, /*in_water*/true});
    REQUIRE(next == kernels::CharStanceK::Water);
}

TEST_CASE("Stance: ladder beats prone but not water",
          "[physics][character][stance]") {
    auto land = kernels::kernel_char_next_stance(
        kernels::CharStanceK::Prone,
        kernels::CharIntent{false, true, /*near_ladder*/true, false});
    REQUIRE(land == kernels::CharStanceK::Ladder);
    auto water = kernels::kernel_char_next_stance(
        kernels::CharStanceK::Ladder,
        kernels::CharIntent{false, false, true, true});
    REQUIRE(water == kernels::CharStanceK::Water);
}

TEST_CASE("Stance: crouch button transitions Stand → Crouch",
          "[physics][character][stance]") {
    auto next = kernels::kernel_char_next_stance(
        kernels::CharStanceK::Stand,
...
TEST_CASE("Stance: prone request from Prone state keeps you prone",
          "[physics][character][stance]") {
    auto next = kernels::kernel_char_next_stance(
        kernels::CharStanceK::Prone,
...
struct StepWorld {
    bool operator()(math::Vec3 p) const {
        ...
        if (p.x >= 1.0f && p.x <= 5.0f && p.y < step_top) return true;
        return false;
    }
    f32 step_top = 0.30f;
};
...
TEST_CASE("Stair-step climbs over a 0.30 m step when step_height = 0.35 m",
    ...
    math::Vec3 result = kernels::kernel_stair_step_climb(
        origin, horiz, step_height, world);
