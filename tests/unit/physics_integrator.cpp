...
TEST_CASE("Free-falling sphere matches s = 0.5 * g * t^2 within 1%",
          "[physics][integrator]") {
    Body b{};
    ...
    for (int i = 0; i < 120; ++i) integrate_step(b, g, 1.0f / 120.0f);
...
TEST_CASE("Static body does not move under gravity", "[physics][integrator]") {
    ...
    for (int i = 0; i < 60; ++i) integrate_step(b, g, 1.0f / 120.0f);
    REQUIRE(b.position.y == Approx(5.0f).margin(1e-5f));
}

TEST_CASE("inertia_sphere matches the analytic formula 2/5 * m * r^2",
          "[physics][inertia]") {
...
TEST_CASE("inertia_box matches the analytic formula m/12 * (a^2 + b^2)",
          "[physics][inertia]") {
...
TEST_CASE("aabb_world conservative AABB grows with rotated box",
          "[physics][shape]") {
