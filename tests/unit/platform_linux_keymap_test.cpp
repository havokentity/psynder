...
// ─── compute_blit_rect ───────────────────────────────────────────────────
TEST_CASE("compute_blit_rect: stretch fills the window exactly", "[platform_linux][blit]") {
    const BlitRect r = compute_blit_rect(1920, 1080, 640, 360, AspectMode::Stretch, ScaleMode::Linear);
    REQUIRE(r.x == 0);
    REQUIRE(r.y == 0);
    REQUIRE(r.w == 1920);
    REQUIRE(r.h == 1080);
...
TEST_CASE("compute_blit_rect: letterbox keeps aspect; 16:9 fits 16:9", "[platform_linux][blit]") {
    const BlitRect r =
        compute_blit_rect(1920, 1080, 640, 360, AspectMode::Letterbox, ScaleMode::Linear);
...
TEST_CASE("compute_blit_rect: letterbox bars when window is wider than fb",
    ...
    const BlitRect r =
        compute_blit_rect(1024, 768, 640, 360, AspectMode::Letterbox, ScaleMode::Linear);
...
TEST_CASE("compute_blit_rect: integer scale snaps to whole multiples", "[platform_linux][blit]") {
    // 1920×1080 window, 640×360 fb → 3× integer scale → 1920×1080 (perfect)
    const BlitRect r =
        compute_blit_rect(1920, 1080, 640, 360, AspectMode::Letterbox, ScaleMode::Integer);
    REQUIRE(r.w == 1920);
    REQUIRE(r.h == 1080);
    REQUIRE(r.x == 0);
    REQUIRE(r.y == 0);
...
TEST_CASE("compute_blit_rect: integer scale picks largest multiple that fits",
    ...
    const BlitRect r =
        compute_blit_rect(1300, 800, 640, 360, AspectMode::Letterbox, ScaleMode::Integer);
    REQUIRE(r.w == 1280);
    REQUIRE(r.h == 720);
...
TEST_CASE("compute_blit_rect: integer scale never goes below 1x", "[platform_linux][blit]") {
    ...
    const BlitRect r =
        compute_blit_rect(100, 100, 640, 360, AspectMode::Letterbox, ScaleMode::Integer);
    REQUIRE(r.w == 640);
    REQUIRE(r.h == 360);
}

TEST_CASE("compute_blit_rect: crop covers the window, clips long axis", "[platform_linux][blit]") {
    ...
    const BlitRect r = compute_blit_rect(1920, 1080, 800, 600, AspectMode::Crop, ScaleMode::Linear);
    REQUIRE(r.w == 1920);
    REQUIRE(r.h == 1440);
    REQUIRE(r.x == 0);
...
TEST_CASE("compute_blit_rect: zero/negative inputs return zeroed rect", "[platform_linux][blit]") {
    REQUIRE(compute_blit_rect(0, 0, 640, 360, AspectMode::Letterbox, ScaleMode::Linear).w == 0);
    REQUIRE(compute_blit_rect(1920, 1080, 0, 0, AspectMode::Letterbox, ScaleMode::Linear).w == 0);
}
