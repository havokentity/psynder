...
TEST_CASE("render_post: rain seed populates the pool", "[render_post][rain]") {
    StreakSystem sys;
    sys.seed(1024, default_params());
    REQUIRE(sys.active_count() == 1024);
}

TEST_CASE("render_post: rain seed clamps to capacity", "[render_post][rain]") {
    StreakSystem sys;
    sys.seed(StreakSystem::kMaxStreaks + 100u, default_params());
    REQUIRE(sys.active_count() == StreakSystem::kMaxStreaks);
}

TEST_CASE("render_post: rain step advances streaks downward", "[render_post][rain]") {
    StreakSystem sys;
    ...
    u32 moved_down = 0;
    for (u32 i = 0; i < 64; ++i) {
        if (sys.at(i).position.y < before[i] + 1e-3f)
            ++moved_down;
    }
    REQUIRE(moved_down >= 60u);
}

TEST_CASE("render_post: rain step recycles streaks below kill plane", "[render_post][rain]") {
    StreakSystem sys;
    auto p = default_params();
    sys.seed(32, p);
...
TEST_CASE("render_post: rain render disabled is a no-op", "[render_post][rain]") {
    const u32 W = 8, H = 8;
    std::vector<HdrPixel> fb(static_cast<usize>(W) * H, HdrPixel{0.3f, 0.4f, 0.5f, 1.0f});
    auto reference = fb;
    Framebuffer hdr_fb;
    hdr_fb.width = W;
    hdr_fb.height = H;
...
TEST_CASE("render_post: rain render modifies pixels when enabled", "[render_post][rain]") {
...
TEST_CASE("render_post: rain streak data size is half a cache line", "[render_post][rain]") {
