...
u32 aniso_ref(const std::vector<u32>& tex, u32 w, u32 h,
              f32 u, f32 v,
              f32 du_dx, f32 du_dy, f32 dv_dx, f32 dv_dy,
              u32 max_anisotropy) {
    ...
    if (lx2 >= ly2) { maj_x = ax_x; maj_y = ax_y; maj2 = lx2; min2 = ly2; }
    else            { maj_x = ay_x; maj_y = ay_y; maj2 = ly2; min2 = lx2; }
    u32 N;
    ...
    if (N == 1) return bilinear_ref(tex, w, h, u, v);
    ...
    if (acc_w <= 0.0f) return bilinear_ref(tex, w, h, u, v);
...
TEST_CASE("aniso_sample: isotropic gradient collapses to bilinear",
          "[raster][aniso]") {
    const u32 W = 16, H = 16;
    ...
    const u32 ani = aniso_ref(tex, W, H, /*u*/ 0.25f, /*v*/ 0.25f,
                              /*du_dx*/ inv_w, /*du_dy*/ 0.0f,
                              /*dv_dx*/ 0.0f,  /*dv_dy*/ inv_w,
                              /*max*/ 8);
...
TEST_CASE("aniso_sample: 8x along x-axis produces grey on 2-texel checker",
          "[raster][aniso]") {
    const u32 W = 64, H = 64;
    auto tex = make_checker(W, H, /*block*/ 2);
    ...
    const u32 ani8 = aniso_ref(tex, W, H, u, v,
...
TEST_CASE("r_anisotropy cvar: out-of-range values clamp; "
