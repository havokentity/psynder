...
TEST_CASE("edge_func is positive on the left of a CCW edge", "[raster][edge]") {
    ...
    const i64 above = edge_func(ax, ay, bx, by, FxQ24_8::from_float(5.0f), FxQ24_8::from_float(-5.0f));
    const i64 below = edge_func(ax, ay, bx, by, FxQ24_8::from_float(5.0f), FxQ24_8::from_float(5.0f));
...
TEST_CASE("top-left fill rule: shared edge between two triangles owns each pixel once",
    ...
    for (i32 y = 0; y < 360; y += 7) {
        for (i32 x = 0; x < 640; x += 7) {
            ...
            const bool in1 = eval_edge0(t1, fx, fy) >= 0 && eval_edge1(t1, fx, fy) >= 0 &&
                             eval_edge2(t1, fx, fy) >= 0;
            const bool in2 = eval_edge0(t2, fx, fy) >= 0 && eval_edge1(t2, fx, fy) >= 0 &&
