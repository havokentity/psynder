...
u32 count_lit(const Image& img) {
    u32 c = 0;
    for (u32 p : img.pixels) if (p != 0xFF000000u) ++c;
    return c;
}

}  // namespace

TEST_CASE("test-mesh helpers return valid geometry",
          "[raster][testmesh]") {
...
TEST_CASE("rasterizer fills the test triangle and writes depth",
          "[raster][tile]") {
    Image img(128, 128);
...
TEST_CASE("submit before begin_frame is a no-op (no crash)",
          "[raster][lifecycle]") {
    auto& r = Rasterizer::Get();
    auto tri = test_mesh::colored_triangle();
    DrawItem d{};
