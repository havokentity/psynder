...
struct Lcg {
    ...
    f32 next_f32() noexcept {
        return static_cast<f32>(next_u32()) * (1.0f / 4294967296.0f);
    }
    f32 next_signed(f32 a, f32 b) noexcept {
        return a + (b - a) * next_f32();
    }
};
...
TEST_CASE("Packet-8 matches scalar across 1000 random rays",
          "[render_rt][packet][fuzz]") {
    Lcg rng(0xC0FFEEull);
        ...
        for (u32 i = 0; i < 8; ++i) {
            if (pkt.occluded[i] != scalar_ref[i]) {
                ...
                INFO("packet " << p << " lane " << i
                     << " origin=(" << rays[i].origin.x << "," << rays[i].origin.y
                     << "," << rays[i].origin.z << ") dir=("
                     << rays[i].direction.x << "," << rays[i].direction.y
                     << "," << rays[i].direction.z << ")"
                     << " scalar=" << scalar_ref[i]
                     << " packet=" << pkt.occluded[i]);
...
TEST_CASE("Packet-8 matches scalar with multi-instance TLAS + random rays",
    ...
    for (u32 p = 0; p < kPackets; ++p) {
        ...
        bool scalar_ref[8];
        for (u32 i = 0; i < 8; ++i) scalar_ref[i] = tlas.occluded(rays[i]);
        trace_shadow_packet(tlas, pkt);
        for (u32 i = 0; i < 8; ++i) {
            INFO("packet " << p << " lane " << i);
