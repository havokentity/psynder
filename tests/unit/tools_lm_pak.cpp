...
TEST_CASE("lm_pak round-trips a small set of blobs", "[tools][lm_pak]") {
    ...
    entries.push_back(make_entry("levels/test.psybsp", "PSBP\x01\x00\x00\x00..."));
    entries.push_back(make_entry("textures/wall.lmt", std::string(1024, '\xAB')));
    entries.push_back(make_entry("audio/clip.lma", std::string("LMA1 sample audio data")));
    entries.push_back(make_entry("meshes/cube.lmm", std::string("LMM1 placeholder mesh body")));
    ...
    for (const auto& in : entries) {
        ...
        auto it = std::find_if(info.entries.begin(), info.entries.end(), [&](const EntryRecord& r) {
            return r.hash == h;
        });
