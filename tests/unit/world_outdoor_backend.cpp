...
TEST_CASE("parse_backend accepts 'mesh' (default for racing tracks)", "[world_outdoor][backend]") {
...
TEST_CASE("parse_backend trims whitespace", "[world_outdoor][backend]") {
    pwo::TerrainBackend b{};
...
TEST_CASE("parse_backend rejects unknown values", "[world_outdoor][backend]") {
...
TEST_CASE("backend_name round-trips through parse_backend", "[world_outdoor][backend]") {
    for (auto bin : {pwo::TerrainBackend::PolygonCDLOD, pwo::TerrainBackend::HeightmapRaymarch}) {
