    ...
    for (std::uint32_t z = 0; z < H; ++z) {
        for (std::uint32_t x = 0; x < W; ++x) {
            const float coarse = pwod::coarse_height_at_lod(desc,
                                                            ...
                                                            0);
            const float fine =
                pwod::height_at_texel(desc, static_cast<std::int32_t>(x), static_cast<std::int32_t>(z));
...
TEST_CASE("apply_vertex_morph is leaf identity at morph_factor=0", "[world_outdoor][cdlod][morph]") {
    const std::uint32_t W = 17, H = 17;
    auto raw = make_heightmap_lod(W, H);
    pwo::HeightmapDesc desc{};
    desc.size_x = W;
...
TEST_CASE("CDLOD stitching stays watertight under same-LOD morph",
    ...
    const std::uint32_t R_verts_x = std::min<std::uint32_t>(pwod::kChunkDim + 1u, W - pwod::kChunkDim);
...
TEST_CASE("compute_chunk_morph picks the right LOD band by distance",
    ...
    auto m0 = pwod::compute_chunk_morph(chunk, psynder::math::Vec3{20.0f, 0.0f, 2.0f}, distances, 3);
    ...
    auto m1 = pwod::compute_chunk_morph(chunk, psynder::math::Vec3{150.0f, 0.0f, 2.0f}, distances, 3);
    REQUIRE(m1.lod_level == 1u);

    // Eye at 1000m → past LOD 2 → clamps to coarsest, morph=1.
    auto mFar =
        pwod::compute_chunk_morph(chunk, psynder::math::Vec3{1000.0f, 0.0f, 2.0f}, distances, 3);
...
TEST_CASE("compute_chunk_morph morph factor is monotone in distance within a band",
    ...
    auto a =
        pwod::compute_chunk_morph(chunk, psynder::math::Vec3{70.0f, 0.0f, 0.0f}, distances, 1, 0.667f);
    auto b =
        pwod::compute_chunk_morph(chunk, psynder::math::Vec3{85.0f, 0.0f, 0.0f}, distances, 1, 0.667f);
    auto c =
        pwod::compute_chunk_morph(chunk, psynder::math::Vec3{95.0f, 0.0f, 0.0f}, distances, 1, 0.667f);
