...
std::vector<std::uint16_t> ramp_heightmap(std::uint32_t w, std::uint32_t h, std::uint16_t slope_per_step) {
...
TEST_CASE("raymarch hits a flat heightmap at the expected distance", "[world_outdoor][raymarch]") {
    ...
    pwod::RayHit hit{};
    const bool ok = pwod::march_ray(desc,
...
TEST_CASE("raymarch hits a linear ramp heightmap correctly", "[world_outdoor][raymarch]") {
...
TEST_CASE("raymarch returns false when the ray misses", "[world_outdoor][raymarch]") {
...
TEST_CASE("raymarch detects already-underground origin as t=0 hit", "[world_outdoor][raymarch]") {
...
TEST_CASE("project_y maps height to screen Y predictably", "[world_outdoor][raymarch]") {
...
TEST_CASE("per-column step advances horizon on a rising silhouette", "[world_outdoor][raymarch]") {
