...
std::vector<std::uint16_t> ramp_heightmap_simd(std::uint32_t w, std::uint32_t h,
...
TEST_CASE("SIMD bilinear sampler agrees with scalar (8-wide)",
...
TEST_CASE("SIMD bilinear sampler agrees with scalar (4-wide)",
          "[world_outdoor][raymarch][simd]") {
    ...
    pwo::HeightmapDesc desc{};
    desc.size_x = W; desc.size_z = H;
    desc.spacing = 2.0f; desc.height_scale = 0.1f;
    desc.heights = raw.data();
...
TEST_CASE("SIMD raymarch advance + early-out flips done_mask once a lane runs off-screen",
    ...
    for (std::uint32_t i = 0; i < 8; ++i) step.new_horizon_y[i] = cb.horizon_y[i];
    ...
    for (std::uint32_t i = 0; i < 8; ++i) step.new_horizon_y[i] = -1;
    pwod::simd_advance_batch8(cb, step);
...
TEST_CASE("SIMD raymarch parity over a sweep of distances (8 lanes × 12 steps)",
    ...
    pwo::HeightmapDesc desc{};
    desc.size_x = W; desc.size_z = H;
    desc.spacing = 1.0f; desc.height_scale = 0.08f;
    desc.heights = raw.data();
