...
psynder::math::Mat4 downward_view(psynder::math::Vec3 eye) {
    return psynder::math::look_at_rh(
        eye,
...
TEST_CASE("TerrainRaymarch::render paints pixels into the bound framebuffer",
    ...
    const psynder::math::Mat4 proj = psynder::math::perspective_rh(
        60.0f * psynder::math::kDegToRad,
    ...
    for (auto px : tfb.pixels) if (px != 0u) ++painted;
...
TEST_CASE("TerrainRaymarch::render is a no-op when no target is bound",
    ...
    const psynder::math::Mat4 view = downward_view(
        psynder::math::Vec3{8.0f, 30.0f, 8.0f});
    const psynder::math::Mat4 proj = psynder::math::perspective_rh(
        60.0f * psynder::math::kDegToRad, 4.0f / 3.0f, 1.0f, 200.0f);
...
TEST_CASE("TerrainRaymarch::render reverts to no-op when target is cleared",
    ...
    const psynder::math::Vec3 eye{
        static_cast<float>(kHm) * 0.5f, 30.0f,
        static_cast<float>(kHm) * 0.5f};
    const psynder::math::Mat4 view = downward_view(eye);
    const psynder::math::Mat4 proj = psynder::math::perspective_rh(
        60.0f * psynder::math::kDegToRad,
    ...
    std::memset(tfb.pixels.data(), 0xAB,
                tfb.pixels.size() * sizeof(std::uint32_t));
    std::memset(tfb.depth.data(),  0xCD,
                tfb.depth.size()  * sizeof(std::uint32_t));
