...
TriHit intersect_tri(math::Vec3 ro, math::Vec3 rd, math::Vec3 v0, math::Vec3 v1, math::Vec3 v2, f32 t_max) {
...
ClosestHit closest_hit_bf(const BakeScene& scene, math::Vec3 ro, math::Vec3 rd, f32 t_max, u32 self_tri) {
    ClosestHit best;
    for (u32 i = 0; i < scene.triangles.size(); ++i) {
        if (i == self_tri)
            continue;
...
math::Vec3 cosine_hemisphere(math::Vec3 normal, f32 u, f32 v) {
    ...
    if (tl > 1e-9f)
        t = math::mul(t, 1.0f / tl);
    math::Vec3 b = math::cross(normal, t);
    return math::add(math::add(math::mul(t, x), math::mul(b, y)), math::mul(normal, z));
}
...
TriBasis build_basis(const BakeTriangle& tri) {
    ...
    if (nlen > 1e-12f)
        n = math::mul(n, 1.0f / nlen);
    f32 e1_len = std::sqrt(math::dot(e1, e1));
    math::Vec3 u_axis = e1;
    if (e1_len > 1e-12f)
        u_axis = math::mul(u_axis, 1.0f / e1_len);
...
bool read_lmlight(std::span<const u8> bytes, BakedAtlas& out, std::string* err) {
    ...
    if (bytes.size() < 16)
        return fail("lmlight header truncated");
    u32 magic = 0;
    read_le<u32>(bytes, 0, magic);
    if (magic != kLmlMagic)
        return fail("lmlight bad magic");
    u32 version = 0;
    read_le<u32>(bytes, 4, version);
    if (version != kLmlVersion)
        return fail("lmlight unsupported version");
    u32 count = 0;
    read_le<u32>(bytes, 8, count);
    usize cursor = 16;
    out.surfaces.clear();
    out.surfaces.resize(count);
    for (u32 i = 0; i < count; ++i) {
        u32 w = 0, h = 0;
        if (!read_le<u32>(bytes, cursor + 0, w))
            return fail("lmlight surf header");
        if (!read_le<u32>(bytes, cursor + 4, h))
            return fail("lmlight surf header");
        cursor += 8;
        BakedSurface s;
        s.width = w;
        ...
        for (usize px = 0; px < s.pixels.size(); ++px) {
            u16 hv = 0;
            if (!read_le<u16>(bytes, cursor, hv))
                return fail("lmlight pixel truncated");
    ...
    while (i < text.size()) {
        ...
        while (p < line.size()) {
            ...
            while (p < line.size() && line[p] != ' ' && line[p] != '\t' && line[p] != '\r')
                ++p;
            if (p > start)
                tok.emplace_back(line.substr(start, p - start));
        }
        if (tok.empty())
            continue;
        auto& cmd = tok[0];
...
int cli_main(int argc, char** argv) {
    ...
    for (int i = 3; i < argc; ++i) {
        ...
        if (k == "--resolution" && i + 1 < argc)
            opt.lightmap_resolution = static_cast<u32>(std::atoi(argv[++i]));
        else if (k == "--bounces" && i + 1 < argc)
            opt.max_indirect_bounces = static_cast<u32>(std::atoi(argv[++i]));
        else if (k == "--samples" && i + 1 < argc)
            opt.indirect_samples_per_bounce = static_cast<u32>(std::atoi(argv[++i]));
    }
    std::string text, err;
    ...
    std::fprintf(stdout,
                 "lm_bake: baked %u surfaces -> %s\n",
                 static_cast<u32>(atlas.surfaces.size()),
                 argv[2]);
    return 0;
}
