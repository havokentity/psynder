...
// in-memory inputs:
bool cook_obj_blob(std::string_view obj_text, std::vector<u8>& lmm_out, std::string* err = nullptr);
bool cook_gltf_blob(std::string_view gltf_text,
                    std::span<const u8> external_buffer,
                    std::vector<u8>& lmm_out,
                    std::string* err = nullptr);
bool cook_png_blob(std::span<const u8> png_bytes, std::vector<u8>& lmt_out, std::string* err = nullptr);
bool cook_wav_blob(std::span<const u8> wav_bytes, std::vector<u8>& lma_out, std::string* err = nullptr);
