...
void emit_face(std::ostringstream& out, math::Vec3 a, math::Vec3 b, math::Vec3 c, std::string_view material) {
    char buf[256];
    std::snprintf(buf,
                  sizeof(buf),
...
void make_icosahedron(std::vector<math::Vec3>& verts, std::vector<std::array<u32, 3>>& tris) {
...
void subdivide_icosahedron(std::vector<math::Vec3>& verts,
    ...
    for (u32 lvl = 0; lvl < levels; ++lvl) {
        ...
        for (const auto& tri : tris) {
            ...
            u32 i_ab = static_cast<u32>(verts.size());
            verts.push_back(m_ab);
            u32 i_bc = static_cast<u32>(verts.size());
            verts.push_back(m_bc);
            u32 i_ca = static_cast<u32>(verts.size());
            verts.push_back(m_ca);
...
std::string tessellate_cylinder(const CurveCylinder& c) {
    ...
    if (axis_len > 1e-9f)
        axis = math::mul(axis, 1.0f / axis_len);
    ...
    // +axis. Pick three corner points on the base ring.
    {
        ...
        math::Vec3 p1 = math::add(base,
                                  math::add(math::mul(u_dir, r * std::cos(a1)),
                                            math::mul(v_dir, r * std::sin(a1))));
        f32 a2 = 2.0f * a1;
        math::Vec3 p2 = math::add(base,
                                  math::add(math::mul(u_dir, r * std::cos(a2)),
    ...
    // Top cap (normal = +axis).
    {
        ...
        f32 a1 = kTwoPi / static_cast<f32>(segs);
        math::Vec3 p1 = math::add(top,
                                  math::add(math::mul(u_dir, r * std::cos(a1)),
                                            math::mul(v_dir, r * std::sin(a1))));
        f32 a2 = 2.0f * a1;
        math::Vec3 p2 = math::add(top,
                                  math::add(math::mul(u_dir, r * std::cos(a2)),
                                            math::mul(v_dir, r * std::sin(a2))));
    ...
    for (u32 i = 0; i < segs; ++i) {
        ...
        math::Vec3 du0 =
            math::add(math::mul(u_dir, r * std::cos(a0)), math::mul(v_dir, r * std::sin(a0)));
        math::Vec3 du1 =
            math::add(math::mul(u_dir, r * std::cos(a1)), math::mul(v_dir, r * std::sin(a1)));
...
std::string tessellate_sphere(const CurveSphere& s) {
    ...
    for (auto& v : verts)
        v = math::add(s.origin, math::mul(v, r));
    for (const auto& tri : tris) {
...
bool try_parse_cylinder(std::string_view s, usize& p, std::string& expansion) {
    ...
    if (s.size() - p < kKey.size())
        return false;
    if (s.substr(p, kKey.size()) != kKey)
        return false;
    p += kKey.size();
    ...
    usize tp = 0;
    while (tp < args.size()) {
        while (tp < args.size() && (args[tp] == ' ' || args[tp] == '\t' || args[tp] == '\r'))
            ++tp;
        usize start = tp;
        while (tp < args.size() && args[tp] != ' ' && args[tp] != '\t' && args[tp] != '\r')
            ++tp;
        if (tp > start)
            tok.emplace_back(args.substr(start, tp - start));
    }
...
std::string expand_curve_brushes(std::string_view text) {
    ...
    while (p < text.size()) {
        ...
        if (first_nws < text.size() && text[first_nws] == '@') {
            ...
            if (try_parse_cylinder(text, probe, expansion) || try_parse_sphere(text, probe, expansion)) {
