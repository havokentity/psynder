...
int main(int argc, char** argv) {
    ...
    if (!read_text_file(argv[1], source)) {
        std::fprintf(stderr, "scene_cook: failed to read %s\n", argv[1]);
        return EXIT_FAILURE;
    }
    ...
    CookScene scene;
    std::string error;
    if (!cook_json(root, scene, error)) {
        std::fprintf(stderr, "scene_cook: %s\n", error.c_str());
        return EXIT_FAILURE;
    }

    const std::vector<u8> blob = write_scene_blob(scene);
    if (!write_binary_file(argv[2], std::span<const u8>{blob.data(), blob.size()})) {
        std::fprintf(stderr, "scene_cook: failed to write %s\n", argv[2]);
        return EXIT_FAILURE;
    }
